...
struct LinkAssignment {
    ...
    unsigned short begin_time[BUSSINE_TYPE_NUM]
                             [3]; // 业务开始时间（每行第一位为时,取值范围为[1-24]，第二位为分,取值范围为[0-60]，第三位为秒,取值范围为[0-60]）；
    unsigned short end_time[BUSSINE_TYPE_NUM]
                           [3]; // 业务结束时间（每行第一位为时,取值范围为[1-24]，第二位为分,取值范围为[0-60]，第三位为秒,取值范围为[0-60]）
    unsigned short frequency[BUSSINE_TYPE_NUM]; // 业务频率，物理量范围为[2.5ms/次-5s/次],取值范围为[1-2000],业务频率=取值*2.5ms/次

    // 一个默认初始化函数，全为0
    LinkAssignment() { memset(this, 0, sizeof(LinkAssignment)); }
};
...
// 网管模块信息上报 24.01.10
struct DataManager {
    int Manager_cost;               // 信令开销统计
    clocktype delay_IdentityUpdate; // 身份更新时间
    int IdentityUpdate_Num;         // 身份更新数目
    DataManager() {
        this->delay_IdentityUpdate = 0;
        this->IdentityUpdate_Num = 0;
        this->Manager_cost = 0;
    }
};
