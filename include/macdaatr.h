...
typedef struct Low_Freq_Packet_Type {
    uint8_t type; // 0：无定义  1：随遇接入请求  2：同意随遇接入请求回复  3：拒绝接入请求恢复  4：普通节点飞行状态信息  5：网管节点飞行状态信息
                  // 6：网管节点通告消息
...
typedef struct highFreqBusinessQueue {                                                // 业务信道业务
    int recv_node;                                                                    // 下一跳节点
...
class MacDaatr {
  ...
  public:
    ...
    A_ang subnet_other_node_ang[SUBNET_NODE_NUMBER_MAX];                 // 子网其他节点角度信息
    bool subnet_other_node_ang_flag[SUBNET_NODE_NUMBER_MAX];             // 是否收到子网其他节点角度信息
