...
#include "./_LSTM_MAC/data.h"
#include "./_LSTM_MAC/lstm.h"
#include "./_LSTM_MAC/saveandbuildLSTM.h"
...
6.进行等号操作时, 需注意, 此类的比特序列会新开辟内存空间, 大小与对应类相等,且里面的值也相同, 而结构体指针会清零, 类类型等同对应类
7.加号操作执行合包功能,注意, 若想将c中的比特序列添加与b比特序列后, 成为新的包,需编写如b +c,b成为新的包载体; 同理, 若顺序相反, 则c + b, c成为新的包载体。成为新载体后,
类型均成为255, b,c皆为MacDaatr_struct_converter类
8.减号操作为拆包操作, 形如a-b,其中b的类型必须为255, 根据想要的包决定a的类型, a为一个空类, 即除了类型以外,其他皆为空, 想要什么类型, 就设置a的对应类型,
比如想要飞行状态信息,就设置类型为3, 若想拆掉包头, 拆掉PDU1,类型为99,拆掉PDU2,类型为100,对应拆得的结果放置于b中,同时, 要想拆包, 必须在b内设置序列总长度!!!!
...
// 返回长度
int MacDaatr_struct_converter::get_length() { return length_; }

// 返回此比特序列长度(从PDU包头得到)type: 0 PDU1包  1 PDU2
int MacDaatr_struct_converter::get_sequence_length(uint8_t *bit_seq) { return ((bit_seq[1] << 8) | (bit_seq[2])); }
...
bool MacDaatr_struct_converter::set_struct(uint8_t *daatr_struc, unsigned char type = 0)
{
    ...
    { // 数据量
        ...
        Link_DataVolume *ptr_temp = new Link_DataVolume;
        Link_DataVolume *ptr_temp2 = (Link_DataVolume *)daatr_struc;
        *ptr_temp = *ptr_temp2;
        Daatr_struct_ptr_ = (uint8_t *)ptr_temp;
        break;
    }
...
bool MacDaatr_struct_converter::set_bit_sequence(uint8_t *bit_sequence, unsigned char type = 0)
{
    ...
    { // 飞行状态信息
        ...
        number = 24; // 50为FlightStatus结构体大小(不考虑内存对齐),24为实际传输大小,
        // 部分飞行状态信息因物理限制
        bit_sequence_ptr_ = new uint8_t[number];
        for (i = 0; i < number; i++)
        {
...
bool MacDaatr_struct_converter::daatr_0_1_to_PDU2()
{
    ...
    // cout << "srcAddr: "  << struct_ptr_temp->srcAddr << endl;
    // cout << "destAddr: "  << struct_ptr_temp->destAddr << endl;
    // cout << "slice_head_identification: "  <<
    // struct_ptr_temp->slice_head_identification << endl; cout <<
    // "fragment_tail_identification: "  <<
    // struct_ptr_temp->fragment_tail_identification << endl; cout << "backup: "
    // << struct_ptr_temp->backup << endl; cout << "mac_headerChecksum: "  <<
    // struct_ptr_temp->mac_headerChecksum << endl; cout << "mac_pSynch: "  <<
    // struct_ptr_temp->mac_pSynch << endl; cout << "mac: "  <<
    // struct_ptr_temp->mac << endl;
    ////////////////////
    flag = true;
    return flag;
}
...
bool MacDaatr_struct_converter::daatr_MFCvector_to_0_1(vector<uint8_t> *MFC_vector_temp, int packetlength)
{
    ...
    //     cout << "(*MFC_vector_temp).size(): " << (*MFC_vector_temp).size() <<
    //     endl; cout << "MFC sequence doesn't match given length";
    //     system("pause");
    // }
...
// 传入的是一个converter类型的01序列, 其组成格式为 若干通过重载 "+"
// 相连的MFC集合 此函数将从 多个MFC的01序列 中拆出来第一个MFC所属的01序列
// 并返回(通过MFC的pktlength字段)
...
bool MacDaatr_struct_converter::daatr_mana_info_to_0_1()
{
    ...
    int number = 1;
    int flag = false;
    if (Daatr_struct_ptr_ == NULL || type_ != 4) // 若待转换结构体为空或者此时结构体类型不为网管节点通告消息,
                                                 // 则直接退出
    {
        return flag;
    }
...
bool MacDaatr_struct_converter::daatr_freq_sense_to_0_1()
{
    ...
    int flag = false;
    int num = 0;
    if (Daatr_struct_ptr_ == NULL || type_ != 5) // 若待转换结构体为空或者此时结构体类型不为频谱感知结果,
                                                 // 则直接退出
    {
        return flag;
    }
...
bool MacDaatr_struct_converter::daatr_build_req_to_0_1()
{
    ...
    int flag = false;
    int num = 0;
    if (Daatr_struct_ptr_ == NULL || type_ != 6) // 若待转换结构体为空或者此时结构体类型不为频谱感知结果,
                                                 // 则直接退出
    {
        return flag;
    }
...
bool MacDaatr_struct_converter::daatr_slottable_to_0_1()
{
    ...
    for (i = 0; i < 50; i++)
    {
        packet[11 * i + 0] = slottable_temp->slot_traffic_execution_new[8 * i + 0].state << 7 | (slottable_temp->slot_traffic_execution_new[8 * i + 0].send_or_recv_node & 0x3f8) >> 3; // 高7, 右移3

        packet[11 * i + 1] = (slottable_temp->slot_traffic_execution_new[8 * i + 0].send_or_recv_node & 0x007) << 5 |                                                                   // 低3, 左移5
                             slottable_temp->slot_traffic_execution_new[8 * i + 1].state << 4 | (slottable_temp->slot_traffic_execution_new[8 * i + 1].send_or_recv_node & 0x3c0) >> 6; // 高4, 右移6

        packet[11 * i + 2] = (slottable_temp->slot_traffic_execution_new[8 * i + 1].send_or_recv_node & 0x03f) << 2 |                                                                   // 低6, 左移2
                             slottable_temp->slot_traffic_execution_new[8 * i + 2].state << 1 | (slottable_temp->slot_traffic_execution_new[8 * i + 2].send_or_recv_node & 0x200) >> 9; // 高1, 右移9
        ...
        packet[11 * i + 4] = (slottable_temp->slot_traffic_execution_new[8 * i + 2].send_or_recv_node & 0x001) << 7 |                                                                   // 低1, 左移7
                             slottable_temp->slot_traffic_execution_new[8 * i + 3].state << 6 | (slottable_temp->slot_traffic_execution_new[8 * i + 3].send_or_recv_node & 0x3f0) >> 4; // 高6, 右移4

        packet[11 * i + 5] = (slottable_temp->slot_traffic_execution_new[8 * i + 3].send_or_recv_node & 0x00f) << 4 |                                                                   // 低4, 左移4
                             slottable_temp->slot_traffic_execution_new[8 * i + 4].state << 3 | (slottable_temp->slot_traffic_execution_new[8 * i + 4].send_or_recv_node & 0x380) >> 7; // 高3, 右移7
        ...
        packet[11 * i + 8] = (slottable_temp->slot_traffic_execution_new[8 * i + 5].send_or_recv_node & 0x003) << 6 |                                                                   // 低2, 左移6
                             slottable_temp->slot_traffic_execution_new[8 * i + 6].state << 5 | (slottable_temp->slot_traffic_execution_new[8 * i + 6].send_or_recv_node & 0x3e0) >> 7; // 高5, 右移5

        packet[11 * i + 9] = (slottable_temp->slot_traffic_execution_new[8 * i + 6].send_or_recv_node & 0x01f) << 3 |                                                                   // 低5, 左移3
                             slottable_temp->slot_traffic_execution_new[8 * i + 7].state << 2 | (slottable_temp->slot_traffic_execution_new[8 * i + 7].send_or_recv_node & 0x300) >> 8; // 高2, 右移8
...
bool MacDaatr_struct_converter::daatr_0_1_to_slottable()
{
    ...
    for (i = 0; i < 50; i++)
    {
        ...
        slottable_temp->slot_traffic_execution_new[8 * i + 2].send_or_recv_node =
            (bit_sequence_ptr_[11 * i + 2] & 0x01) << 9 | (bit_sequence_ptr_[11 * i + 3] & 0xff) << 1 | (bit_sequence_ptr_[11 * i + 4] & 0x80) >> 7; // 低1左移9, 中8左移1, 高1右移7
...
bool MacDaatr_struct_converter::daatr_access_reply_to_0_1()
{
    ...
    int number = 32;
    int flag = false;
    if (Daatr_struct_ptr_ == NULL || type_ != 9) // 若待转换结构体为空或者此时结构体类型不为网管节点通告消息,
                                                 // 则直接退出
    {
        return flag;
    }
...
bool MacDaatr_struct_converter::daate_link_datavolume_to_0_1()
{
    int number = 4; // 10 + 10 + 12 = 32bit
    int flag = false;
    int i;
    if (Daatr_struct_ptr_ == NULL || type_ != 11)
    {
        return flag;
    }
    ...
    Link_DataVolume *data_volume_temp = (Link_DataVolume *)Daatr_struct_ptr_;
    uint8_t *packet = new uint8_t[number]; // 生成新空间

    packet[0] = (data_volume_temp->srcAddr & 0x3fc) >> 2;
    packet[1] = (data_volume_temp->srcAddr & 0x003) << 6 | (data_volume_temp->destAddr & 0x3f0) >> 4;
    packet[2] = (data_volume_temp->destAddr & 0x00f) << 4 | (data_volume_temp->data_volume & 0xf00) >> 8;
    packet[3] = data_volume_temp->data_volume & 0x0ff;
...
bool MacDaatr_struct_converter::daate_link_0_1_to_datavolume()
{
    int number = 4;
    int flag = false;

    if (bit_sequence_ptr_ == NULL || type_ != 11)
    {
        return flag;
    }
    if (Daatr_struct_ptr_ != NULL)
    ...
    Link_DataVolume *data_volume_temp = new Link_DataVolume;
    memset(data_volume_temp, 0, sizeof(Link_DataVolume));

    data_volume_temp->srcAddr = (bit_sequence_ptr_[0] << 2) | (bit_sequence_ptr_[1] & 0xc0) >> 6;
    data_volume_temp->destAddr = (bit_sequence_ptr_[1] & 0x3f) << 4 | (bit_sequence_ptr_[2] & 0xf0) >> 4;
    data_volume_temp->data_volume = (bit_sequence_ptr_[2] & 0x0f) << 8 | bit_sequence_ptr_[3];

    // cout << "srcAddr: " << data_volume_temp->srcAddr << endl;
    // cout << "destAddr: " << data_volume_temp->destAddr << endl;
    // cout << "data_volume: " << data_volume_temp->data_volume << endl;

    Daatr_struct_ptr_ = (uint8_t *)data_volume_temp;
    flag = true;
    return flag;
}
...
bool MacDaatr_struct_converter::daatr_struct_to_0_1()
{
    ...
    case 10:
        ...
        break;
    case 11:
        flag = daate_link_datavolume_to_0_1();
        break;
    }
    return flag;
}
...
bool MacDaatr_struct_converter::daatr_0_1_to_struct()
{
    ...
    case 10:
        ...
        break;
    case 11:
        flag = daate_link_0_1_to_datavolume();
        break;
    }
    return flag;
}
...
// 根据子网分配跳频点生成子网内使用的频段
static void Generate_Subnet_Used_Freqency(MacDataDaatr *macdata_daatr)
{
    int i, j;
    int temp = 0;
    for (i = 0; i < TOTAL_FREQ_POINT; i++)
...
// 跳频序列调整
static void Adjust_Frequency_Sequence(unsigned int frequency_sequence[SUBNET_NODE_FREQ_NUMBER][FREQUENCY_COUNT], const unsigned char available_frequency[TOTAL_FREQ_POINT])
{
    int flag = 0;
    for (int time = 0; time < FREQUENCY_COUNT; time++)
    {
...
static void Generate_Frequency_Sequence(MacDataDaatr *macdata_daatr)
{
    ...
    // unsigned int
    // node_frequency[SUBNET_NODE_NUMBER][FREQUENCY_COUNT];//调整后序列
...
static void MacDaatrInitialize_Frequency_Seqence(Node *node, MacDataDaatr *macdata_daatr)
{
        ...
        while (narrow_band_found == 1 && total_attempts < max_attempts)
        {
            ...
            for (int time = 0; time < FREQUENCY_COUNT; time++)
            {
                for (int node = 0; node < SUBNET_NODE_FREQ_NUMBER; node++)
                {
                    ...
                    while (prev_node >= 0)
                    {
                        if (((frequency_sequence[node][time] - frequency_sequence[prev_node][time] >= 0) &&
                             (frequency_sequence[node][time] - frequency_sequence[prev_node][time] < MIN_FREQUENCY_DIFFERENCE)) ||
                            ((frequency_sequence[node][time] - frequency_sequence[prev_node][time] < 0) &&
                             (frequency_sequence[prev_node][time] - frequency_sequence[node][time] < MIN_FREQUENCY_DIFFERENCE)))
...
static void LSTM_Generate_predict_data()
{
    // 测试各个节点到 节点1 的链路数据预测量
    for (int src = 2; src <= SUBNET_NODE_NUMBER; src++)
    {
        string lstm_model_name = "..\\..\\..\\..\\libraries\\user_models\\src\\_LSTM_MAC\\Model_para\\lstmmodel_" + to_string(static_cast<long long>(src)) + ".txt";

        char lstm_model_name_c[100];
        strcpy(lstm_model_name_c, lstm_model_name.c_str());
        Lstm *lstm = BuildLSTMModelFromPlainText(lstm_model_name_c);

        // for (int dest = 1; dest <= SUBNET_NODE_NUMBER; dest++)
        // {
        int dest = 1;
        string lstm_training_data = "..\\..\\..\\..\\libraries\\user_models\\src\\_LSTM_MAC\\lstm_"
                                    "dataset_per_link\\data_volume_" +
                                    to_string(static_cast<long long>(src)) + "_" + to_string(static_cast<long long>(dest)) + "_" + to_string(static_cast<long long>(COLLECT_DATA_VOLUME_PERIOD)) +
                                    ".csv";
        vector<double> data = read_csv(lstm_training_data); // 存储二维数组的容器

        vector<double> diff_data;
        vector<double> norm_data;
        double max = 10000.0, min = 0.0;
        // 数据归一化处理
        norm_data = fixed_normalize(data, max, min);
        data = norm_data;

        double *testinput = new double[INPUT];
        for (int i = data.size() - INPUT; i < data.size(); i++)
        {
            testinput[i - (data.size() - INPUT)] = data[i]; // 输出
            // cout << denormalize_s(data[i], max, min) << " ";
        }
        // cout << endl;
        double *outpredict = lstm->predict(testinput);
        double outpredict_real = denormalize_s(outpredict[0], max, min);
        if (outpredict_real <= 0)
        {
            outpredict_real = 0;
        }
        // cout << src << "->" << dest << " Pridict Data: " << outpredict_real << endl;
        // }

        string predict_data = "..\\..\\..\\..\\libraries\\user_models\\src\\_LSTM_MAC\\predict_data\\predict_data_" + to_string(static_cast<long long>(src)) + "_" +
                              to_string(static_cast<long long>(dest)) + "_" + to_string(static_cast<long long>(COLLECT_DATA_VOLUME_PERIOD)) + ".csv";
        ofstream fout(predict_data, ios::app);

        if (!fout.is_open())
        {
            cout << "Could Not Open Predict_Data File" << endl;
            system("pause");
        }

        fout << outpredict_real << endl;
    }
}

// 返回任意两节点间距离(单位: km)
static double Calculate_Distance_Between_Nodes(float lat_1, float lng_1, float h_1, float lat_2, float lng_2, float h_2)
{
...
static double Calculate_Transmission_Rate_by_Distance(double distance)
{
    double tran_speed = 0;
    if (distance <= 50)
    {
...
static void Send_Local_Link_Status(Node *node, MacDataDaatr *macdata_daatr)
{
    ...
    {
        if (macdata_daatr->traffic_channel_business[dest_node - 1].distance != 0 && macdata_daatr->if_receive_mana_flight_frame[dest_node - 1] == true)
        {
            Capacity = Calculate_Transmission_Rate_by_Distance(macdata_daatr->traffic_channel_business[dest_node - 1].distance);
    ...
    // cout << "子网内存在的其他节点个数为: " << send_neighborList->size() <<
    // endl;
    ...
    Message *data_msg = MESSAGE_Alloc(node, NETWORK_LAYER, ROUTING_PROTOCOL_MMANET, MSG_ROUTING_ReceiveLocalLinkMsg);
...
bool MacDaatr_network_has_mana_channel_packet_to_send(Node *node, int interfaceIndex, msgFromControl *busin)
{
    int i = 0;
    int j = 0;
...
bool get_mana_channel_business_from_queue(msgFromControl *busin, Node *node, MacDataDaatr *macdata_daatr)
{
    int i = 0;
    bool flag = false;
    ...
    {                                                                      // 证明当前队列有业务
        *busin = macdata_daatr->mana_channel_business_queue[i].busin_data; // 将当前队列中的业务赋值给输入的空白业务
        ...
        cout << "已成功从网管信道队列中取出业务," << "目的地址为: " << busin->destAddr << endl;
        flag = true;
    }
    else
    {
...
// 返回俯仰角 方位角 等天线指向信息
static A_ang Get_Angle_Info(float lat_1, float lng_1, float h_1, short int pitch_1, short int roll_1, short int yaw_1, float lat_2, float lng_2, float h_2)
{
    A_ang a_ang[5];
    // 计算5个天线对应的俯仰角和方位角等信息
...
static void Attach_Status_Information_To_Packet(Node *node, Node *dest_node, MacDataDaatr *macdata_daatr, unsigned short RX_TX_Flag, clocktype delay)
{
    ...
    A_ang dest_node_angle = Get_Angle_Info(macdata_daatr->local_node_position_info.positionX, macdata_daatr->local_node_position_info.positionY, macdata_daatr->local_node_position_info.positionZ,
                                           macdata_daatr->local_node_position_info.pitchAngle, macdata_daatr->local_node_position_info.rollAngle, macdata_daatr->local_node_position_info.yawAngle,
                                           macdata_daatr->subnet_other_node_position[temp_Id].positionX, macdata_daatr->subnet_other_node_position[temp_Id].positionY,
    ...
    int temp_distance = Calculate_Distance_Between_Nodes(macdata_daatr->local_node_position_info.positionX, macdata_daatr->local_node_position_info.positionY,
                                                         macdata_daatr->local_node_position_info.positionZ, macdata_daatr->subnet_other_node_position[temp_Id].positionX,
                                                         macdata_daatr->subnet_other_node_position[temp_Id].positionY, macdata_daatr->subnet_other_node_position[temp_Id].positionZ);
    // 需要 distance 到 level 的转换
    Status_Information->transmitSpeedLevel = Calculate_Transmission_Rate_by_Distance(temp_distance);
    // 需要跳频图案
    ...
    // 暂时使用 MAC 层的协议
    Message *flight_status_msg = MESSAGE_Alloc(node, MAC_LAYER, MAC_PROTOCOL_DAATR, MSG_MAC_Attach_Information_To_PHY);
...
// 计算子网各节点窄带干扰数目
// type: 0:更新前  1：更新后
static void MacDaatr_Cacu_node_narrow_band_interfer_ratio(Node *node, MacDataDaatr *macdata_daatr, int type)
{
    int i = 0, j = 0, k;
    ...
    vector<ParamData> param_narrow_list; // 节点窄带干扰比例
    ...
    if (type == 1)
    {
        for (i = 0; i < SUBNET_NODE_NUMBER; i++)
        {
            ...
            pa_narrow_data_str.first = i + 1; /////统计参数上传
            for (j = i + 1; j < SUBNET_NODE_NUMBER; j++)
            {
            ...
            param_narrow_list.push_back(pa_narrow_data); /////统计参数上传
...
#ifdef ISCAS_LIB
        // Stats_ProSendData(node, 3, 1, 11, param_narrow_list); // 跳频图案自扰比例
        // cout << "已向上层发送 MAC 跳频图案自扰比例(触发式, 指标11), Time: " <<
        // (float)(node->getNodeTime()) / 1000000 << endl;

        // Stats_ProSendData(node, 3, 1, 12, ParamDataStr_narrow_list); //
        // 互扰节点编号 cout << "已向上层发送 MAC 互扰节点编号(触发式, 指标12),
        // Time: " << (float)(node->getNodeTime()) / 1000000 << endl;
#endif
    }
    if (type == 0)
    {
...
static void Judge_If_Enter_Freq_Adjust(Node *node, MacDataDaatr *macdata_daatr)
{
    ...
    */
    ...
    for (i = 1; i <= SUBNET_NODE_NUMBER; i++)
    {
        for (j = 0; j < FREQUENCY_COUNT; j++)
        {
            temp_freq = macdata_daatr->frequency_sequence[i - 1][j];
            if (macdata_daatr->unavailable_frequency_points[temp_freq] == 0)
            {
                node_interfer_number++;
            }
        }
        macdata_daatr->stats.node_freq_interfer_ratio_before[i - 1] = ((float)node_interfer_number / FREQUENCY_COUNT) * 100;
        // macdata_daatr->stats.node_freq_interfer_ratio_before[i - 1] *= 100;
        node_interfer_number = 0;
    }

    ///////////////////////////////////////向上传递干扰信息参数//////////////////////////////////
    vector<ParamData> param_list; // 节点干扰比例
    ParamData pa_data;
    pa_data.nodeId = 0; // 子网干扰比例
    pa_data.value = macdata_daatr->stats.subnet_freq_interfer_ratio_before;
    param_list.push_back(pa_data);
    for (i = 1; i <= SUBNET_NODE_NUMBER; i++)
    {
        pa_data.nodeId = i; // 节点干扰比例
        pa_data.value = macdata_daatr->stats.node_freq_interfer_ratio_before[i - 1];
        param_list.push_back(pa_data);
    }

#ifdef ISCAS_LIB
    // Stats_ProSendData(node, 3, 1, 13, param_list); // 频段被干扰比例
    // cout << "已向上层发送 MAC 频段被干扰比例(触发式, 指标13), Time: " <<
    // (float)(node->getNodeTime()) / 1000000 << endl;
#endif
    ...
    for (i = 1; i <= SUBNET_NODE_NUMBER; i++)
    {                          // 其他节点干扰频点
        pa_data_str.first = i; // 节点干扰比例
        for (j = 0; j < TOTAL_FREQ_POINT; j++)
        {
            if (macdata_daatr->spectrum_sensing_sum[i - 1][j] == 0)
            {                                                     // 如果此频点被干扰
...
#ifdef ISCAS_LIB
    // Stats_ProSendData(node, 3, 1, 14, ParamDataStr_list); // 被干扰频点
    // cout << "已向上层发送 MAC 被干扰频点(触发式, 指标14), Time: " <<
    // (float)(node->getNodeTime()) / 1000000 << endl;
#endif
    ...
    if (using_freq_interfer_ratio >= INTERFEREENCE_FREQUENCY_THRESHOLD)
    {
        ...
        macdata_daatr->stats.freq_adjust_begin_time = (float)(node->getNodeTime()) / 1000000;
        Generate_Frequency_Sequence(macdata_daatr);
        Generate_Subnet_Used_Freqency(macdata_daatr);
        ...
        { // 在这之前已经发生了要进入时隙调整阶段, 但还未广播此信息,
            // 也即未进入
            cout << "Now Is About To Enter Slot Adjustment Stage, Can't Enter Freq "
                    "Adjustment Stage! "
                 << endl;
            macdata_daatr->need_change_stage = 3; // 设置为首先进入时隙调整阶段,
            // 在时隙调整阶段结束后进入频率调整阶段
        }
        ...
        // 在收到链路构建需求事件MAC_DAATR_ReceiveTaskInstruct之后 500ms
        // 后发送时隙表
        Message *prepare_msg = MESSAGE_Alloc(node, MAC_LAYER, MAC_PROTOCOL_DAATR, MSG_MAC_ManaPrepareToSendSequence);
        clocktype current_frametime = node->getNodeTime();
        clocktype delay;
        if (((current_frametime / 1000000) % 1000) < 520)
        {
            delay = (520 * MILLI_SECOND - current_frametime % (1000 * MILLI_SECOND) + 100 * MILLI_SECOND);
        }
        // else if (((current_frametime / 1000000) % 1000) >= 20 &&
        // ((current_frametime / 1000000) % 1000) < 520)
        // {
        //     delay = (520 * MILLI_SECOND - current_frametime % (1000 *
        //     MILLI_SECOND) + 100 * MILLI_SECOND);
        // }
        else
        {
            delay = (1000 * MILLI_SECOND - current_frametime % (1000 * MILLI_SECOND) + 100 * MILLI_SECOND);
        }

        MESSAGE_Send(node, prepare_msg, delay);

        // MESSAGE_Send(node, prepare_msg, SENDING_SLOTTABLE_PREPARE_TIME *
        // MILLI_SECOND);
    }
    else
    {
...
static void Print_Wave_Info(FlightStatus node_flight, FlightStatus nei_flight[SUBNET_NODE_NUMBER - 1], int nodeId)
{
    ...
    for (int i = 0; i < SUBNET_NODE_FREQ_NUMBER - 1; i++)
    {
        a_ang[i] = Get_Angle_Info(node_flight.positionX, node_flight.positionY, node_flight.positionZ, node_flight.pitchAngle, node_flight.rollAngle, node_flight.yawAngle, nei_flight[i].positionX,
                                  nei_flight[i].positionY, nei_flight[i].positionZ);
...
static void Update_Position(MacPacket_Daatr *macpacket_daatr, MacDataDaatr *macdata_daatr)
{
    for (int i = 0; i < SUBNET_NODE_FREQ_NUMBER - 1; i++)
    { // 存储子网其他节点位置信息
        if (macdata_daatr->subnet_other_node_position[i].nodeId == 0)
    ...
    macdata_daatr->traffic_channel_business[node_temp - 1].distance =
        Calculate_Distance_Between_Nodes(macdata_daatr->local_node_position_info.positionX, macdata_daatr->local_node_position_info.positionY, macdata_daatr->local_node_position_info.positionZ,
                                         macpacket_daatr->node_position.positionX, macpacket_daatr->node_position.positionY, macpacket_daatr->node_position.positionZ);
}
...
{
    return (LinkAssignment_single_temp->begin_node == macdata_daatr->gateway_node || LinkAssignment_single_temp->end_node == macdata_daatr->gateway_node ||

            LinkAssignment_single_temp->begin_node == macdata_daatr->standby_gateway_node || LinkAssignment_single_temp->end_node == macdata_daatr->standby_gateway_node);
    // 暂定网关节点为2
}
...
static bool Judge_If_Choose_Slot(LinkAssignment_single *LinkAssignment_single_temp, Alloc_slot_traffic Alloc_slottable_traffic[], int slot_length)
{
    for (int i = 0; i < slot_length; i++)
    {
        ...
        // 计算当前时隙已经进行多少个复用
        {
            if (LinkAssignment_single_temp->begin_node == Alloc_slottable_traffic[i].send_node[j] || LinkAssignment_single_temp->end_node == Alloc_slottable_traffic[i].recv_node[j] ||
                LinkAssignment_single_temp->begin_node == Alloc_slottable_traffic[i].recv_node[j] || LinkAssignment_single_temp->end_node == Alloc_slottable_traffic[i].send_node[j])
            {
...
static void Calculate_slotnum_per_frame(Node *node, MacDataDaatr *macdata_daatr, LinkAssignment_single *LinkAssignment_single_temp)
{
    int i, node1, node2;
    ...
    // LinkAssignment_single_temp->end_node == node->nodeId)
    // {
    if (LinkAssignment_single_temp->begin_node == node->nodeId)
    {
    ...
    // LinkAssignment_single_temp->distance =
    // std::max({LinkAssignment_single_temp->distance, 500});
...
static mana_slot_traffic Generate_mana_slot_traffic(Alloc_slot_traffic *Alloc_slottable_traffic_temp)
{
    ...
    // memset(&Alloc_slottable_traffic, 0, sizeof(Alloc_slottable_traffic) *
    // TRAFFIC_SLOT_NUMBER);
    ...
    for (int i = 0; i < SUBNET_NODE_FREQ_NUMBER; i++)
    {
        for (int j = 0; j < TRAFFIC_SLOT_NUMBER; j++)
        {
            if (Alloc_slottable_traffic[j].multiplexing_num != FREQUENCY_DIVISION_MULTIPLEXING_NUMBER)
            {
                ...
                {
                    // Alloc_slottable_traffic[j].send_node 是 网管节点 的分配表的第 j
                    // 个时隙的发送节点序列 send[k] 是其中的第 k 组链路需求的发送节点
                    // 需要找到20个时隙表中的第senk[k]个,
                    // 然后将其第[j]个时隙的state和send进行修改 修改为TX,
                    // 并将send指向recv节点
...
static Alloc_slot_traffic *ReAllocate_Traffic_slottable(Node *node, MacDataDaatr *macdata_daatr, vector<LinkAssignment_single> LinkAssignment_Storage)
{
    int i, j, k, m;
    // int break_flag = 0;
    ...
    Alloc_slot_traffic Alloc_slottable_traffic[TRAFFIC_SLOT_NUMBER];
    // Alloc_slot_traffic *Alloc_slottable_traffic = new
    // Alloc_slot_traffic[TRAFFIC_SLOT_NUMBER];
    ...
    // 全连接
    if (SUBNET_NODE_NUMBER <= 10)
    {
        int i = 0, j = 0;
        int slot_loc = 0; // 用来标识目前正在分配时隙的位置, 非子网间时隙才可以分

        string Full_connection_send = "..\\..\\..\\..\\libraries\\user_models\\src\\FullConnection\\FullConnection_10_send.txt";
        string Full_connection_recv = "..\\..\\..\\..\\libraries\\user_models\\src\\FullConnection\\FullConnection_10_recv.txt";
    ...
    else if (SUBNET_NODE_NUMBER > 10 && SUBNET_NODE_NUMBER <= 15)
    {
        int i = 0, j = 0;
        int slot_loc = 0; // 用来标识目前正在分配时隙的位置, 非子网间时隙才可以分

        string Full_connection_send = "..\\..\\..\\..\\libraries\\user_models\\src\\FullConnection\\FullConnection_15_send.txt";
        string Full_connection_recv = "..\\..\\..\\..\\libraries\\user_models\\src\\FullConnection\\FullConnection_15_recv.txt";
        ...
        while (i < 30)
        {
            if (slot_loc % 20 < 8 || slot_loc % 20 >= 16)
            {
                for (j = 0; j < 7; j++)
                {
                    Alloc_slottable_traffic[slot_loc].send_node[j] = send_node[7 * i + j];
                    Alloc_slottable_traffic[slot_loc].recv_node[j] = recv_node[7 * i + j];

                    rest_slot_num_per_node[send_node[7 * i + j] - 1] -= 1;
                    rest_slot_num_per_node[recv_node[7 * i + j] - 1] -= 1;
                }
                Alloc_slottable_traffic[slot_loc].multiplexing_num = 0;
                slot_loc++;
                i++;
    ...
    for (la = LinkAssignment_Storage.begin(); la != LinkAssignment_Storage.end(); la++)
    {
        ...
        Calculate_slotnum_per_frame(node, macdata_daatr, &(*la)); // 计算LA_single的时隙数
    ...
    for (i = 0; i < TRAFFIC_SLOT_NUMBER; i++)
    {
        ...
        if (Alloc_slottable_traffic[i].multiplexing_num == FREQUENCY_DIVISION_MULTIPLEXING_NUMBER) // 此时隙未分配任何链路
        {
            ...
            for (int ii = 1; ii <= SUBNET_NODE_NUMBER; ii++)
            {
        ...
        else
        {
            ...
            if (i % 20 >= 8 && i % 20 < 16)
            {
                for (j = 0; j < temp; j++)
                {
                    for (k = 1; k <= SUBNET_NODE_NUMBER; k++)
                    {
                        if (k != Alloc_slottable_traffic[i].send_node[j] && k != Alloc_slottable_traffic[i].recv_node[j] && k != macdata_daatr->gateway_node &&
            ...
            else
            {
                for (j = 0; j < temp; j++)
                {
                    for (k = 1; k <= SUBNET_NODE_NUMBER; k++)
                    {
                        if (k != Alloc_slottable_traffic[i].send_node[j] && k != Alloc_slottable_traffic[i].recv_node[j])
                        {
...
static vector<LinkAssignment> *Generate_LinkAssignment_Initialization(MacDataDaatr *macdata_daatr)
{
    int i, j;
...
// 搜寻对应节点时隙位置
static vector<uint8_t> *Search_Slot_Location(int nodeId, MacDataDaatr *macdata_daatr)
{
    int i = 0;
...
static void MacDaatrInitialize_pkt_Counter(Node *node, MacDataDaatr *macdata_daatr)
{
    ...
    macdata_daatr->stats.send_recv_slot_num = 0;

    macdata_daatr->stats.slot_adjust_num = 0;
    macdata_daatr->stats.freq_adjust_num = 0;

    // Message *pkt_counter_timerMsg = MESSAGE_Alloc(node, MAC_LAYER,
    // MAC_PROTOCOL_DAATR, MSG_MAC_PktCounter); MESSAGE_Send(node,
    // pkt_counter_timerMsg, SIMULATION_RESULT_SHOW_PERIOD * MILLI_SECOND);
}
...
{
    Message *send_msg = MESSAGE_Alloc(node, MAC_LAYER, MAC_PROTOCOL_DAATR, MSG_MAC_End_Link_Build);
...
{
    Message *send_msg0 = MESSAGE_Alloc(node, MAC_LAYER, MAC_PROTOCOL_DAATR, MAC_DAATR_SendOtherNodePosition);
...
static void MacDaatrInitialize_parameter(Node *node, MacDataDaatr *macdata_daatr)
{
    ...
    ifstream fin;
    fin.open("..\\..\\..\\..\\libraries\\user_models\\src\\ManaNodeConfigue\\mac_"
             "daatr.txt",
             ios::in);
    int mana_node;
    int gateway_node;
...
{
    Message *wave_msg = MESSAGE_Alloc(node, MAC_LAYER, MAC_PROTOCOL_DAATR, MSG_MAC_Beam_maintenance);
    MESSAGE_Send(node, wave_msg, BEAM_MATAINMENCE_PERIOD * MILLI_SECOND);
}

static void MacDaatrInitialize_LSTM(Node *node, MacDataDaatr *macdata_daatr)
{
    for (int i = 1; i <= SUBNET_NODE_NUMBER; i++)
    {
        macdata_daatr->stats.throughput_per_link[i - 1] = 0;

        macdata_daatr->link_data_volume_stats[i - 1].srcAddr = node->nodeId;
        macdata_daatr->link_data_volume_stats[i - 1].destAddr = i;
        macdata_daatr->link_data_volume_stats[i - 1].data_volume = 0;
    }
    ...
    {
        string cmd = "del /q "
                     "..\\..\\..\\..\\libraries\\user_models\\src\\_LSTM_MAC\\predict_data\\*.csv";
        char cmd_c[100];
    ...
    {
        string cmd = "del /q "
                     "..\\..\\..\\..\\libraries\\user_models\\src\\_LSTM_MAC\\lstm_dataset_per_link\\*.csv";
        char cmd_c[100];
        strcpy(cmd_c, cmd.c_str());
        system(cmd_c);
    }

    Message *data_volume_msg = MESSAGE_Alloc(node, MAC_LAYER, MAC_PROTOCOL_DAATR, MSG_MAC_LSTM_CollectlinkDataVolume);
    MESSAGE_Send(node, data_volume_msg, COLLECT_DATA_VOLUME_PERIOD * MILLI_SECOND);
}
...
static void MacDaatrInitialize_Judge_If_Enter_Freq_Adjust_Timer(Node *node, MacDataDaatr *macdata_daatr)
{
    if (node->nodeId == macdata_daatr->mana_node)
    {
        ...
        MESSAGE_Send(node, send_msg, 20000 * MILLI_SECOND - 1 * NANO_SECOND);
...
static void MacDaatrInitialize_LocalLinkState_timer(Node *node, MacDataDaatr *macdata_daatr)
{
    ...
    Message *send_msg = MESSAGE_Alloc(node, MAC_LAYER, MAC_PROTOCOL_DAATR, MSG_MAC_SendLocalLinkStateTimer);
...
{
    Message *send_msg0 = MESSAGE_Alloc(node, MAC_LAYER, MAC_PROTOCOL_DAATR, Mac_Management_Channel_Period_Inquiry);
...
static void MacDaatrInitialize_traffic_Timer(Node *node, MacDataDaatr *macdata_daatr)
{
    ...
    if (i == TRAFFIC_SLOT_NUMBER)
    {
        ...
        if (DEBUG)
        {
            printf("No initial timer sent for node %d. Initial Status matches all slots "
                   "in frame\n",
                   node->nodeId);
        }
    }
    else
    {
        ...
        timerMsg = MESSAGE_Alloc(node, MAC_LAYER, MAC_PROTOCOL_DAATR, MSG_MAC_Traffic_Channel_Period_Inquiry);
...
static void MacDaatrInitialize_Load_PreLaunch_Slottable(Node *node, MacDataDaatr *macdata_daatr)
{
    if (node->nodeId == macdata_daatr->mana_node)
    {
        int i, j, k;
        ...
        vector<LinkAssignment>::iterator LA_in_list;

        for (LA_in_list = link_assign_queue->begin(); LA_in_list != link_assign_queue->end(); LA_in_list++)
        {
            for (j = 0; j < (*LA_in_list).listNumber; j++)
            {
                ...
                LinkAssignment_single_temp.begin_time[3] = (*LA_in_list).begin_time[3][j];
                LinkAssignment_single_temp.end_time[3] = (*LA_in_list).end_time[3][j];
        ...
        for (int i = 0; i < SUBNET_NODE_FREQ_NUMBER; i++)
        {
            for (int j = 0; j < TRAFFIC_SLOT_NUMBER; j++)
            {
                if (Alloc_slottable_traffic[j].multiplexing_num != FREQUENCY_DIVISION_MULTIPLEXING_NUMBER)
                {
                    // for (int k = 0; k < FREQUENCY_DIVISION_MULTIPLEXING_NUMBER -
                    // Alloc_slottable_traffic[j].multiplexing_num; k++)
                    for (int k = 0; k < FULL_CONNECTION_NODE_NUMBER / 2; k++)
        ...
        for (dest_node = 1; dest_node <= SUBNET_NODE_NUMBER; dest_node++)
        {
            ...
            string stlotable_state_filename = "..\\..\\..\\..\\libraries\\user_models\\src\\Slottable_"
                                              "Initialization\\Slottable_RXTX_State_" +
                                              to_string(static_cast<long long>(dest_node)) + ".txt";
            string stlotable_node_filename = "..\\..\\..\\..\\libraries\\user_models\\src\\Slottable_"
                                             "Initialization\\Slottable_RXTX_Node_" +
                                             to_string(static_cast<long long>(dest_node)) + ".txt";
            ...
            for (i = 0; i < TRAFFIC_SLOT_NUMBER; i++)
            {
                if (macdata_daatr->slot_traffic_execution_new[dest_node - 1][i].state == DAATR_STATUS_TX && macdata_daatr->slot_traffic_execution_new[dest_node - 1][i].send_or_recv_node != 0)
                {
                    fout2 << macdata_daatr->slot_traffic_execution_new[dest_node - 1][i].send_or_recv_node << ",";
                }
                else if (macdata_daatr->slot_traffic_execution_new[dest_node - 1][i].state == DAATR_STATUS_RX && macdata_daatr->slot_traffic_execution_new[dest_node - 1][i].send_or_recv_node != 0)
                {
                    fout2 << macdata_daatr->slot_traffic_execution_new[dest_node - 1][i].send_or_recv_node << ",";
                }
                else
    ...
    Message *slottable_loading_msg = MESSAGE_Alloc(node, MAC_LAYER, MAC_PROTOCOL_DAATR, MSG_MAC_Read_Slottable_From_Txt);
...
static void MacDaatrInitialize_Link_Setup_Timer(Node *node, MacDataDaatr *macdata_daatr)
{
    ...
    if (macdata_daatr->mana_node == 1)
    {
        ...
        {
            Message *Link_Setup_msg = MESSAGE_Alloc(node, MAC_LAYER, MAC_PROTOCOL_DAATR, MSG_MAC_Start_Link_Build);
...
void MacDaatrInit(Node *node, int interfaceIndex, const NodeInput *nodeInput, const int subnetListIndex, const int numNodesInSubnet)
{
    int i;
    ...
    // for (i = 0; i < MANAGEMENT_SLOT_NUMBER_OTHER_STAGE; i++)
    // {
    //     macdata_daatr->slot_management_other_stage[i].state =
    //     other_stage_mana_slot_table_state_seq[i];
    //     macdata_daatr->slot_management_other_stage[i].nodeId =
    //     other_stage_mana_slot_table_nodeId_seq[i];
    // }
    ...
    //////////////////////////////////// 定时器初始化 ///////////////////////////////////////
    MacDaatrInitialize_parameter(node, macdata_daatr); // 节点必要参数初始化, 目前里面内容可省略,
    // 若添加非零初始化, 不可省略
    MacDaatrInitialize_mana_Timer(node, macdata_daatr); // 网管信道定时器初始化
    // MacDaatrInitialize_Wave_Timer(node);                //    // 波束维护模块定时器初始化
    MacDaatrInitialize_LSTM(node, macdata_daatr); // LSTM 模块初始化
    ...
    MacDaatrInitialize_Frequency_Seqence(node, macdata_daatr);                // 初始化跳频序列
    MacDaatrInitialize_Judge_If_Enter_Freq_Adjust_Timer(node, macdata_daatr); // 如果是网管节点, 则初始化定期判断是否进入频率调整阶段
    MacDaatrInitialize_Load_PreLaunch_Slottable(node, macdata_daatr);         // 生成并通知装载射前时隙表
    //////////////////////////////////// 定时器初始化 ///////////////////////////////////////
    ...
    // for (i = 0; i < TOTAL_FREQ_POINT; i++)
    // {
    //     spec_struct.spectrum_sensing[i] = Randomly_Generate_0_1(0.9);
    // }
    // Message *spec_sense_msg = MESSAGE_Alloc(node,
    //                                         MAC_LAYER,
    //                                         MAC_PROTOCOL_DAATR,
    //                                         MAC_DAATR_SpectrumSensing);
    // MESSAGE_PacketAlloc(node, spec_sense_msg, sizeof(spectrum_sensing_struct),
    // TRACE_DAATR); memcpy((spectrum_sensing_struct
    // *)MESSAGE_ReturnPacket(spec_sense_msg),
    //        &spec_struct, sizeof(spectrum_sensing_struct));
    // MESSAGE_Send(node, spec_sense_msg, 12000 * MILLI_SECOND);
    /////////////////////////////// 生成频谱感知结果测试 //////////////////////////////////
}
...
// 将业务信道的业务队列前移一格(第一个业务发送完成, 消除其队列)
// prio:      优先级对应的需要前移的队列(0--(TRAFFIC_CHANNEL_PRIORITY-1))
// recv:      某接收节点对应的优先级
// dest_node: 对应的节点(1-20), 所以需要减一
// location:  位置(0-(TRAFFIC_MAX_BUSINESS_NUMBER-1))
static void Business_Lead_Forward(MacDataDaatr *macdata_daatr, int dest_node, int prio, int location)
{
    int i = location;
    // for (i = location; i < TRAFFIC_MAX_BUSINESS_NUMBER - 1; i++)
    while (macdata_daatr->traffic_channel_business[dest_node - 1].business[prio][i + 1].priority != -1 && i < TRAFFIC_MAX_BUSINESS_NUMBER - 1)
    {
        macdata_daatr->traffic_channel_business[dest_node - 1].business[prio][i] = macdata_daatr->traffic_channel_business[dest_node - 1].business[prio][i + 1]; // 对应优先级队列往前移一位,
        // 同时也完成了指定业务的移除
        i++;
    }
...
static int Search_Empty_Location_of_Queue(MacDataDaatr *macdata_daatr, int dest_node, int pri)
{
    int i;
...
// temp: 0:
// 将pri优先级的网管信道业务队列中的location所指的业务提升一个优先级并放入高一级优先级的队列中,
// 且优先级减1, 若高一级优先级队列中的业务已满, 则不做任何操作 temp: 1:
// 将此业务移动到最高优先级队列的第一个, 并将原队列前移一位
// (此时根据实际情景不可能出现最高优先级队列已满的情况,
// 对于已在最高优先级的业务不做任何处理, 因为实际上其已是第一个)
static int Business_Priority_Adjustment(MacDataDaatr *macdata_daatr, int dest_node, int location, int pri, int temp)
{
    int num = 0;
    int type = 1;
    if (temp == 0)
    {
        num = Search_Empty_Location_of_Queue(macdata_daatr, dest_node,
                                             pri - 1); // 搜寻高一级优先级队列中是否还有空余位置
        ...
        {                                                                                                      // 如果还有空闲位置
            ...
            macdata_daatr->traffic_channel_business[dest_node - 1].business[pri - 1][num] = macdata_daatr->traffic_channel_business[dest_node - 1].business[pri][location];
            Business_Lead_Forward(macdata_daatr, dest_node, pri,
                                  location); // 消除此位置的业务, 并前移
            type = 0;                        // 代表前移成功
        }
    }
    else
    {
        macdata_daatr->traffic_channel_business[dest_node - 1].business[0][0] = macdata_daatr->traffic_channel_business[dest_node - 1].business[pri][location];
        macdata_daatr->traffic_channel_business[dest_node - 1].business[0][0].priority = 0;
        Business_Lead_Forward(macdata_daatr, dest_node, pri,
                              location); // 消除当前业务所在队列中的该业务, 并前移一位
        type = 0;                        // 代表成功
    }
    return type;
}

// 对节点 node (0-19) 的第 pri 个优先级队列进行检查, 找到队列中非空的第一个位置
// loc 并返回 dest_node: 1 - SUBNET_NODE_NUM dest_node = -1
// 时表示检查网管信道队列
static int Get_Available_Location_of_Queue(MacDataDaatr *macdata_daatr, int dest_node, char pri)
{
    int loc = 0;
    if (dest_node != -1)
    {
...
static void Insert_MFC_to_Queue(MacDataDaatr *macdata_daatr, msgFromControl *busin, int pri, int loc, int temp, int my_pkt_flag)
{
    ...
    if (my_pkt_flag == 0)
    {
        ...
        else
        {
            ...
            for (i = loc; i > 0; i--)
            {
                macdata_daatr->traffic_channel_business[busin->destAddr - 1].business[pri][i] = macdata_daatr->traffic_channel_business[busin->destAddr - 1].business[pri][i - 1];
            }
    ...
    else // 这说明这是需要查看路由表后再插入队列的数据包
    {
        ...
        else
        {
            ...
            for (i = loc; i > 0; i--)
            {
                macdata_daatr->traffic_channel_business[next_hop_addr - 1].business[pri][i] = macdata_daatr->traffic_channel_business[next_hop_addr - 1].business[pri][i - 1];
            }
...
static void MFC_Priority_Adjustment(MacDataDaatr *macdata_daatr)
{
    ...
    // for (i = 1; i <= SUBNET_NODE_NUMBER; i++)
    // {
    //     for (j = 1; j < TRAFFIC_CHANNEL_PRIORITY; j++) //
    //     对于已经处在最高优先级的业务不做处理
    //     {
    //         int empty_loc = Search_Empty_Location_of_Queue(macdata_daatr, i,
    //         j); for (k = 0; k < empty_loc; k++)
    //         {
    //             if (macdata_daatr->traffic_channel_business[i -
    //             1].business[j][k].waiting_time > 1)
    //             {
    //                 if (Business_Priority_Adjustment(macdata_daatr, i, 0, j,
    //                 0))
    //                 {
...
static bool Judge_If_Include_ReTrans(msgFromControl MFC_temp, vector<msgFromControl> MFC_list_temp)
{
    ...
    {
        if ((*mfc_in_list).packetLength == MFC_temp.packetLength && (*mfc_in_list).seq == MFC_temp.seq && (*mfc_in_list).msgType == MFC_temp.msgType && (*mfc_in_list).priority == MFC_temp.priority &&
            (*mfc_in_list).fragmentNum == MFC_temp.fragmentNum && (*mfc_in_list).fragmentSeq == MFC_temp.fragmentSeq)
        {
            return true;
        }
    }
...
bool MAC_NetworkLayerHasPacketToSend(Node *node, int interfaceIndex, msgFromControl *busin)
{
    ...
    */
    ...
    int next_hop_node;
    MacDataDaatr *macdata_daatr = (MacDataDaatr *)node->macData[interfaceIndex]->macVar;
    if ((busin->packetLength == 73 && busin->msgType == 15 && busin->fragmentNum == 15) || (busin->packetLength == 90 && busin->msgType == 14 && busin->fragmentNum == 14))
    {
        // 如果是链路层自己的数据包, 那么不需要经过转发表
        loc = Get_Available_Location_of_Queue(macdata_daatr, busin->destAddr, busin->priority);
    }
    else
    {
        // 如果是网络层数据包, 那么需要查看路由表对应节点的队列是否已满
        next_hop_node = Search_Next_Hop_Addr(macdata_daatr, busin->destAddr);

        if (busin->msgType == 2)
        {
            next_hop_node = 0;
            // loc = Get_Available_Location_of_Queue(macdata_daatr, busin->destAddr,
            // busin->priority);
        }
        ...
        {
            loc = Get_Available_Location_of_Queue(macdata_daatr, next_hop_node, busin->priority);
            // cout << "节点 " << node->nodeId << " 收到目的地址为: " <<
            // busin->destAddr << " 的数据包, 下一跳转发节点为 " << next_hop_node <<
            // endl;
            //  system("pause");
        }
    }
    ...
    if (loc == TRAFFIC_MAX_BUSINESS_NUMBER) // 这说明队列全满
    {
        ...
        if ((busin->packetLength == 73 && busin->msgType == 15 && busin->fragmentNum == 15) ||
            (busin->packetLength == 90 && busin->msgType == 14 && busin->fragmentNum == 14)) // 这是频谱感知结果数据包
        {
            // 在此处, 说明优先级0的队列已经全满, 只能丢弃最后一个业务,
            // 腾出第一个位置来放置频谱感知结果 loc - 1 = 9, 将 1-9 的业务后移一位
        ...
        else if (busin->msgType == 2)
        {
            ...
            {
                new_loc = Get_Available_Location_of_Queue(macdata_daatr, busin->destAddr, i);
                if (new_loc != TRAFFIC_MAX_BUSINESS_NUMBER)
                {
                    Insert_MFC_to_Queue(macdata_daatr, busin, i, new_loc, 0,
                                        0); // 不查看路由表
                    flag = 1;
                    break;
                }
        ...
        else
        {
            ...
            for (i = busin->priority + 1; i < TRAFFIC_CHANNEL_PRIORITY; i++)
            {
                new_loc = Get_Available_Location_of_Queue(macdata_daatr, Search_Next_Hop_Addr(macdata_daatr, busin->destAddr), i);
                if (new_loc != TRAFFIC_MAX_BUSINESS_NUMBER)
                {
...
void MacDaatrLayer(Node *node, int interfaceIndex, Message *msg)
{
    ...
    case MSG_MAC_Receivemsg_From_Phy:
    {
        ...
        //		slot_num_temp--;
        //	}
        //	macdata_daatr->slot_management_other_stage[slot_num_temp].nodeId
        //= mac_header2.srcAddr;
        // }
        ...
        { // 链路层数据包
            ...
            { // 随遇接入请求
                ...
                { // 如果本节点是网管节点
                    ...
                    macdata_daatr->access_state = DAATR_WAITING_TO_REPLY; // 网管节点已收到接入请求,
                    // 等待下一接入请求恢复时隙回复
            ...
            { // 同意随遇接入请求
                if (node->nodeId == mac_header2.destAddr)
                {
                    ...
                    macdata_daatr->access_state = DAATR_WAITING_TO_ACCESS; // 节点已收到网管节点同意请求,
                    // 等待接收全网跳频图案
                    macdata_daatr->mana_node = mac_header2.srcAddr; // 此时的请求回复的源地址即为网管节点, 重新更新
                    macdata_daatr->slot_management_other_stage[0].nodeId = mac_header2.srcAddr;
                    macdata_daatr->slot_management_other_stage[26].nodeId = mac_header2.srcAddr; // 若网管信道时隙表更改，此处更改(25)!!!!!!!!!
                    Message *send_msg_access_waiting_time = MESSAGE_Alloc(node, MAC_LAYER, MAC_PROTOCOL_DAATR,
            ...
            { // 拒绝随遇接入请求
                if (node->nodeId == mac_header2.destAddr)
                {
                    ...
                    macdata_daatr->access_state = DAATR_NEED_ACCESS; // 节点已收到网管节点拒绝接入请求,
                    // 重新等待接入
            ...
            { // 普通节点飞行状态信息
                ...
                Update_Position(&macpacket_daatr, macdata_daatr);
                ...
                //	if(macdata_daatr->slot_management_other_stage[macdata_daatr->mana_slottable_synchronization].state
                //== DAATR_STATUS_ACCESS_REQUEST
                //		||
                // macdata_daatr->slot_management_other_stage[macdata_daatr->mana_slottable_synchronization].state
                //== DAATR_STATUS_ACCESS_REPLY
                //		||
                // macdata_daatr->slot_management_other_stage[macdata_daatr->mana_slottable_synchronization].nodeId
                //== node->nodeId)
                ...
                //		int slot_number =
                // macdata_daatr->mana_slottable_synchronization;
                //		macdata_daatr->slot_management_other_stage[slot_number].nodeId
                //= mac_header2.srcAddr;
                //	}
                //	else
                //	{//结束同步
                //		macdata_daatr->mana_slottable_synchronization = 0;
                //		macdata_daatr->if_mana_slottable_synchronization =
                // false;
                //	}
                //
                // }
            ...
            {
                // 第一次网管节点通告消息
                // 网管节点的macdata_daatr->state_now
                // 在"MAC_DAATR_ReceiveTaskInstruct"中更改 非网管节点的 在此处更改,
                // 接收网管节点的消息后根据数据包内容更改
                ...
                {
                    // cout << "节点 " << node->nodeId << " 此时未收到状态调整信息.
                    // Time: " << (float)(node->getNodeTime()) / 1000000 << endl;
                }
        ...
        else if (mac_header2.backup == 0)
        {
            ...
            // 0122 changed
            Message *msg_send_network = MESSAGE_Alloc(node, NETWORK_LAYER, ROUTING_PROTOCOL_MMANET, MSG_ROUTING_ReceiveMsgFromLowerLayer);
        ...
        // 在建链阶段时 网管节点每收到别的节点的广播消息,
        // 就说明此时网管节点该把波束对准该节点 (Reserved) if
        // (macdata_daatr->state_now == Mac_Initialization && node->nodeId ==
        // macdata_daatr->mana_node)
        // {
        //     Node *dest_node =
        //     MAPPING_GetNodePtrFromHash(node->partitionData->nodeIdHash,
        //     mac_header2.srcAddr); Attach_Status_Information_To_Packet(node,
        //     dest_node, macdata_daatr, DAATR_STATUS_RX, 20); cout << "[建链阶段]
        //     网管节点将天线对准 Node " << mac_header2.srcAddr << " ,
        //     等待接收建链请求 "; cout << "Time: " <<
        //     (float)(node->getNodeTime()) / 1000000 + 20 << " ms" << endl;
        // }
        // system("pause");
    ...
    case Mac_Management_Channel_Period_Inquiry:
    {
        // 网管信道定期发送数据包事件
        // 抛出下一个20ms事件
        Message *send_msg = MESSAGE_Alloc(node, MAC_LAYER, MAC_PROTOCOL_DAATR, Mac_Management_Channel_Period_Inquiry);
        ...
        if (node->isValid == false)
        {
            ...
            else
            {
                ...
                for (int i = 0; i < MANA_MAX_BUSINESS_NUMBER; i++)
                {
                    macdata_daatr->mana_channel_business_queue[i].state = 0;
                }
            }
            MESSAGE_Free(node, msg);
            break;
        ...
        { // 如果当前阶段为建链阶段
            ...
            if (macdata_daatr->slot_management[slot_num].nodeId == node->nodeId && macdata_daatr->slot_management[slot_num].state == DAATR_STATUS_FLIGHTSTATUS_SEND)
            {
                cout << endl;
                ...
                Message *send_msg0 = MESSAGE_Alloc(node, MAC_LAYER, MAC_PROTOCOL_DAATR,
                ...
                for (i = 1; i <= SUBNET_NODE_NUMBER; i++)
                {
                    ...
                    { // 向各节点(除本节点)外发送信息(广播)
                        // 发送自己节点的位置信息
                        Message *send_msg1 = MESSAGE_Alloc(node, MAC_LAYER, MAC_PROTOCOL_DAATR,
                ...
                macdata_daatr->stats.mac_network_overhead += 42; // 建链阶段 广播飞行状态信息
            }
            else
            { // 为接收时隙
                // 向本节点物理层传输网管节点参数(发)
        ...
        { // 如果为其他阶段
            ...
            if (macdata_daatr->slot_management_other_stage[slot_num].nodeId == node->nodeId ||
                macdata_daatr->slot_management_other_stage[slot_num].state == 3)
            { // 此时隙为发送时隙或者此时隙为接入时隙
                Management_Channel_Information temp_Management_Channel_Information;
                temp_Management_Channel_Information.SendOrReceive = 1;
                temp_Management_Channel_Information.transmitSpeedLevel = 1;
                ...
                { // 接入请求
                    if (macdata_daatr->state_now != Mac_Access)
                    { // 节点未处于接入状态, 不需要广播接入请求
                        // cout << "[接入广播-MANA] Node ID " << node->nodeId << "
                        // 未处于接入状态, 非建链阶段不需要广播 网管信道 接入请求 ";
                        // float time = (float)(node->getNodeTime()) / 1000000;
                        // cout << "Time : " << time << "ms" << endl;
                        temp_Management_Channel_Information.SendOrReceive = 2;
                    }
                    else if (macdata_daatr->access_state == DAATR_NEED_ACCESS && macdata_daatr->state_now == Mac_Access)
                    {
                        ...
                        { // 如果等待退避数目时隙数为0
                            ...
                            for (i = 1; i <= SUBNET_NODE_NUMBER; i++)
                            {
                                ...
                                { // 向各节点(除本节点)外发送信息(广播)
                                    ...
                                    mac_header2_ptr->PDUtype = 1;
                                    mac_header2_ptr->srcAddr = node->nodeId;
                                    mac_header2_ptr->destAddr = 0x3ff;                 // 目的地址为网管节点
                                    ...
                                    node_ptr_to_node = MAPPING_GetNodePtrFromHash(node->partitionData->nodeIdHash,
                                                                                  i); // 获取接收节点指针
                                    MESSAGE_Send(node_ptr_to_node, send_msg1, 0);
                                    delete mac_header2_ptr;
                                }
                            ...
                            macdata_daatr->stats.mac_network_overhead += 18; // 节点向网管节点发送建链请求
                        ...
                        { // 等待退避数减1
                            ...
                            cout << endl;
                            cout << "[接入广播-MANA] Node ID " << node->nodeId << " 处于退避状态, 还需要退避 " << (int)macdata_daatr->access_backoff_number << " 接入时隙";
                            float time = (float)(node->getNodeTime()) / 1000000;
                            cout << "Time : " << time << "ms" << endl;
                        }
                    }
                ...
                {       // 接入回复
                    if (macdata_daatr->state_now == Mac_Access)
                    {
                        cout << "网管节点 " << node->nodeId << " 处于接入阶段，停止收发" << endl;
                        break;
                    }
                    ...
                    { // 如果网管节点有需要回复的节点
                        ...
                        slot_location = Search_Slot_Location(macdata_daatr->waiting_to_access_node, macdata_daatr);
                        ...
                        for (i = 0; i < FREQUENCY_COUNT; i++)
                        {
                            ...
                            // access_reply.mana_node_hopping[i]
                            // = i;//测试用
                        }
                        for (i = 0; i < 5; i++)
                        {
                        ...
                        macdata_daatr->stats.mac_network_overhead += 50; // 网管节点回复建链请求
                        ...
                        //    for (j = 0; j < FREQUENCY_COUNT; j++)
                        //    {
                        //        subnet_parttern_str.subnet_node_hopping[i][j] =
                        //        macdata_daatr->frequency_sequence[i][j];
                        //    }
                        //}
                        // MacDaatr_struct_converter subnet_parttern_ptr(10);
                        // subnet_parttern_ptr.set_struct((uint8_t
                        // *)&subnet_parttern_str, 10);
                        ...
                        // MFC_temp->destAddr = macdata_daatr->waiting_to_access_node;
                        // // 给网管节点发送 MFC_temp->backup = 1; // 标识本层MFC数据包
                        ...
                        // cout << "节点 " << node->nodeId << "
                        // 将结果存入最高优先级的业务队列中. Time: " <<
                        // (float)(node->getNodeTime()) / 1000000 << endl;
                        // MAC_NetworkLayerHasPacketToSend(node, 0, MFC_temp); //
                        // 将此业务添加进业务信道队列
                    ...
                    { // 不处于执行阶段, 拒绝接入
                        ...
                        macdata_daatr->stats.mac_network_overhead += 18; // 网管节点 回复拒绝建链请求
                ...
                { // 飞行状态信息通告
                    if (macdata_daatr->state_now == Mac_Access)
                    {
                        cout << "Node " << node->nodeId << " 处于接入阶段，停止收发" << endl;
                        break;
                    }
                    cout << endl;
                    ...
                    macdata_daatr->stats.mac_network_overhead += 42; // 执行阶段 广播飞行状态信息
                    break;
                }
                case 6:
                {
                    ...
                    macdata_daatr->stats.mac_network_overhead += 19; // 执行阶段广播通告消息
                    break;
                }
                case 7:
                { // 网络层业务发送
                    ...
                    bool flag;
                    flag = get_mana_channel_business_from_queue(&buss_to_be_sent, node, macdata_daatr);
                    if (flag == true)
                    { // 如果业务队列不为空, 有待发送业务
                        ...
                        for (i = 1; i <= SUBNET_NODE_NUMBER; i++)
                        {
                            ...
                            { // 向各节点(除本节点)外发送信息
                                ...
                                mac_converter1.set_struct((uint8_t *)mac_header2_ptr, 2);
                                mac_converter1.daatr_struct_to_0_1();
                                MacDaatr_struct_converter mac_converter2(255); // 设定混合类型的converter,
                                // 用来容纳MFC转换的01序列的seq
                        ...
                        Message *send_inditity = MESSAGE_Alloc(node, NETWORK_LAYER, NETWORK_PROTOCOL_IDENTITYUPDATEANDMAINTENANCE,
    ...
    case MAC_DAATR_Access_Request_Waiting_Time:
    {
        ...
        { // 如果节点尚未收到网管节点请求接入回复
            ...
            cout << "等待接入请求回复时间超过阈值, 结束等待阶段, 重新进入调整状态, "
                    "执行随机退避策略"
                 << endl;
            int num = rand() % (MAX_RANDOM_BACKOFF_NUM + 1);
    ...
    case MSG_MAC_Traffic_Channel_Period_Inquiry:
    {
            ...
            for (i = 0; i < SUBNET_NODE_FREQ_NUMBER; i++)
            {
                for (j = 0; j < TRAFFIC_CHANNEL_PRIORITY; j++)
                {
                    for (k = 0; k < TRAFFIC_MAX_BUSINESS_NUMBER; k++)
                    {
                        ...
                        {
                            macdata_daatr->traffic_channel_business[i].business[j][k].priority = -1;
                        }
                    }
                }
            }
        ...
        bool full_connection_slot = false;

        // 全连接时隙
        if (SUBNET_NODE_NUMBER <= 10) // 18
        {
            if ((current_slot_Id % 20 < 8 || current_slot_Id % 20 >= 16) && current_slot_Id < 26)
            {
                full_connection_slot = true;
            }
        }
        else if (SUBNET_NODE_NUMBER > 10 && SUBNET_NODE_NUMBER <= 15) // 30
        {
            if ((current_slot_Id % 20 < 8 || current_slot_Id % 20 >= 16) && current_slot_Id < 46)
            {
                full_connection_slot = true;
            }
        }
        ...
        {
            if ((current_slot_Id % 20 < 8 || current_slot_Id % 20 >= 16) && current_slot_Id < 62)
            {
                full_connection_slot = true;
            }
        }
        ...
        if (macdata_daatr->state_now == Mac_Execution || macdata_daatr->state_now == Mac_Initialization)
        {
            if (current_slot_state == DAATR_STATUS_TX)
            {
                ...
                if (macdata_daatr->access_state == DAATR_WAITING_TO_SEND_HOPPING_PARTTERN && macdata_daatr->waiting_to_access_node == dest_node && full_connection_slot == true)
                // 接入阶段 向新节点发送新跳频图案
                {
                    int j;
                    subnet_frequency_parttern subnet_parttern_str;
                    ...
                    Message *send_msg = MESSAGE_Alloc(node, MAC_LAYER, MAC_PROTOCOL_DAATR,
                    ...
                    cout << "网管节点向接入节点发送跳频图案 ";
                    float time = (float)(node->getNodeTime()) / 1000000;
                    cout << "Time : " << time << " ms" << endl;

                    macdata_daatr->stats.mac_network_overhead += mac_header->packetLength; // 接入阶段 网管节点向新节点发送跳频图案

                    macdata_daatr->access_state = DAATR_NO_NEED_TO_ACCESS;
                    macdata_daatr->waiting_to_access_node = 0;
                }

                trans_speed = Calculate_Transmission_Rate_by_Distance(macdata_daatr->traffic_channel_business[dest_node - 1].distance);
                trans_data = trans_speed * TRAFFIC_SLOT_DURATION / 8;

                trans_data -= MAC_HEADER1_LENGTH; // 减去PDU1的包头数据量,
                // 才是真正一帧可传输的数据量
                for (i = 0; i < TRAFFIC_CHANNEL_PRIORITY; i++)
                {
                    // i对应不同优先级
                    ...
                    for (j = 0; j < TRAFFIC_MAX_BUSINESS_NUMBER; j++)
                    {
                        ...
                        // 有任何一个条件不满足, 则停止遍历, 并将此前的所有业务打包发送
                        {
                                ...
                                Judge_If_Include_ReTrans(macdata_daatr->traffic_channel_business[dest_node - 1].business[i][skip_pkt].busin_data, MFC_list_temp) == false)
                            {
                                ...
                                Business_Lead_Forward(macdata_daatr, dest_node, i, skip_pkt); // 此业务发送成功, 更新当前业务队列
                            }
                            else
                            {
                                skip_pkt += 1;
                        ...
                        {
                            macdata_daatr->stats.send_recv_slot_num++;
                            ...
                            {
                                Message *spectrum_msg = MESSAGE_Alloc(node, MAC_LAYER, MAC_PROTOCOL_DAATR,
                                ...
                                mac_header->destAddr = macdata_daatr->mana_node;
                                mac_header->srcAddr = node->nodeId;
                                ...
                                mac_converter_Spec.set_length(busin_data.packetLength); // 设定此converter的长度为MFC的长度
                                // 73(固定长度)
                                ...
                                Node *node_ptr_to_node = MAPPING_GetNodePtrFromHash(node->partitionData->nodeIdHash,
                                                                                    macdata_daatr->mana_node); // 获取该接收节点指针

                                Business_Lead_Forward(macdata_daatr, dest_node, i, 0);
                                ...
                                macdata_daatr->stats.mac_network_overhead += mac_header->packetLength; // 执行阶段 其他节点发送频谱感知结果
                            }
                            else if (busin_data.packetLength == 90 && busin_data.msgType == 14 && busin_data.fragmentNum == 14)
                            {
                                Message *data_volume_msg = MESSAGE_Alloc(node, MAC_LAYER, MAC_PROTOCOL_DAATR,
                                                                         MAC_DAATR_Mana_Node_Recv_Data_Volume); // 生成消息

                                // 设置包头
                                MacHeader *mac_header = new MacHeader;
                                mac_header->is_mac_layer = 0;
                                mac_header->backup = 1;
                                cout << "节点 " << node->nodeId << " 向网管节点发送链路数据. Time: " << (float)(node->getNodeTime()) / 1000000 << endl;
                                mac_header->PDUtype = 0;
                                mac_header->packetLength = 90 + 25; // MFC_Spec + 包头25
                                mac_header->destAddr = macdata_daatr->mana_node;
                                mac_header->srcAddr = node->nodeId;
                                ...
                                // 包头后的内容
                                vector<uint8_t> *MFC_DV_temp = new vector<uint8_t>; // 存储盛放频谱感知结果的MFC的vector序列
                                MFC_DV_temp = PackMsgFromControl(&busin_data);      // 将MFC转换为 01 seq, 使用vector存放
                                MacDaatr_struct_converter mac_converter_DV(255);
                                // 设定converter, 类别为255
                                // (由于没有header + MFC的type, 所以设为255)
                                mac_converter_DV.set_length(busin_data.packetLength); // 设定此converter的长度为MFC的长度
                                // 73(固定长度)
                                mac_converter_DV.daatr_MFCvector_to_0_1(MFC_DV_temp, busin_data.packetLength); // 将MFC的vector加入converter的seq中

                                mac_converter_PDU1 + mac_converter_DV; // 将两个包拼接进 PDU

                                uint8_t *frame_ptr = mac_converter_PDU1.get_sequence(); // 将拼接好的PDU的seq提出 待发送
                                MESSAGE_PacketAlloc(node, data_volume_msg, mac_header->packetLength, TRACE_DAATR);
                                memcpy(MESSAGE_ReturnPacket(data_volume_msg), frame_ptr, mac_header->packetLength);
                                ...
                                Attach_Status_Information_To_Packet(node, node_ptr_to_node, macdata_daatr, DAATR_STATUS_TX, 0);
                                MESSAGE_Send(node_ptr_to_node, data_volume_msg, 0);
                            }
                            else // 不可能进入此else
                            {
                                Message *data_msg = MESSAGE_Alloc(node, MAC_LAYER, MAC_PROTOCOL_DAATR, MSG_MAC_Control_Transmitting);
                                ...
                                MESSAGE_PacketAlloc(node, data_msg, data_msg_packetSize, TRACE_DAATR);
                                memcpy(MESSAGE_ReturnPacket(data_msg), macdata_pkt, sizeof(MacPacket_Daatr));                    // 查询路由表
                ...
                {
                    Message *data_msg = MESSAGE_Alloc(node, MAC_LAYER, MAC_PROTOCOL_DAATR, MSG_MAC_Data_Transmitting);

                    int upper_layer_control_data_volume = 0;
                    ...
                    for (mfc_t = MFC_list_temp.begin(); mfc_t != MFC_list_temp.end(); mfc_t++)
                    {
                        ...
                        if (mfc_t->msgType == 2 || mfc_t->msgType == 4)
                        {
                            upper_layer_control_data_volume += mfc_t->packetLength;
                        }
                    }

                    // 在自己的模块内记录 并累加发送数据量
                    macdata_daatr->stats.throughput_per_link[dest_node - 1] += mac_header->packetLength;

                    // macdata_daatr->link_data_volume_stats[dest_node - 1].data_volume += mac_header->packetLength;
                    macdata_daatr->link_data_volume_stats[dest_node - 1].data_volume += upper_layer_control_data_volume;
                    // upper_layer_control_data_volume = 0;
                    ...
                    int PDU_TotalLength = mac_converter_PDU1.get_length(); // PDU_TotalLength ==
                    // mac_header->packetLength
                    for (mfc_temp = MFC_list_temp.begin(); mfc_temp != MFC_list_temp.end(); mfc_temp++)
                    {
                        ...
                        // macdata_daatr->stats.mac_network_overhead +=
                        // (*mfc_temp).packetLength;
                    ...
                    Attach_Status_Information_To_Packet(node, dest_node_struct, macdata_daatr, DAATR_STATUS_TX, 0);
                    MESSAGE_Send(dest_node_struct, data_msg, 1 * NANO_SECOND); // 设定一纳秒的延时，是为了确保收方在收到此消息时已经完成了定时器的更新
        ...
        else if (macdata_daatr->state_now == Mac_Adjustment_Slot)
        {
            if (macdata_daatr->link_build_state == 0) // 在建链阶段读取完时隙表
            {
                ...
                cout << "节点 " << node->nodeId << " 取消原有定时器事件, 等待进入执行阶段. Time: " << (float)(node->getNodeTime()) / 1000000 << endl;
            }
            else
            {
                // 节点在 20 520 收到网管节点的通告消息, 更改自己的state_now
                MESSAGE_CancelSelfMsg(node, macdata_daatr->timerMsg);
                cout << "节点 " << node->nodeId << " 进入时域调整阶段, 取消原有定时器事件, Time: " << (float)(node->getNodeTime()) / 1000000 << endl;
        ...
        {
            // cout << "节点 " << node->nodeId << " 进入接入状态, Time: " <<
            // (float)(node->getNodeTime()) / 1000000 << endl;
    ...
    // 只有网管节点(nodeId == mana_node)可以收到这个事件
    // 由网管节点的网络层下发
    case MAC_DAATR_ReceiveTaskInstruct:
    {
        ...
        else if (macdata_daatr->need_change_stage == 2)
        {
            ...
            macdata_daatr->need_change_stage = 4; // 设置为首先进入频率调整阶段,
            // 在时隙调整阶段结束后进入时隙调整阶段
            macdata_daatr->stats.slot_adjust_begin_time = (float)(node->getNodeTime()) / 1000000; // 时隙调整阶段开始时间
        }

        // 在网管节点的网管信道下一发送时隙到来时
        // 向所有节点发送MSG_MAC_Receivemsg_From_Phy事件
        ...
        // LSTM 生成链路数据
        for (int src = 1; src <= SUBNET_NODE_NUMBER; src++)
        {
            string lstm_model_name = "..\\..\\..\\..\\libraries\\user_models\\src\\_LSTM_MAC\\Model_para\\lstmmodel_" + to_string(static_cast<long long>(src)) + ".txt";
            ...
            for (int dest = 1; dest <= SUBNET_NODE_NUMBER; dest++)
            {
                if (src != dest)
                {
                    string lstm_training_data = "..\\..\\..\\..\\libraries\\user_models\\src\\_LSTM_MAC\\lstm_"
                                                "dataset_per_link\\data_volume_" +
                                                to_string(static_cast<long long>(src)) + "_" + to_string(static_cast<long long>(dest)) + "_" +
                                                to_string(static_cast<long long>(COLLECT_DATA_VOLUME_PERIOD)) + ".csv";
                    vector<double> data = read_csv(lstm_training_data); // 存储二维数组的容器
                    ...
                    // 查看单步预测的结果(testnum)向后testnum个数据
                    // 单步预测
                    ...
                    LinkAssignment_single LinkAssignment_single_temp;
                    LinkAssignment_single_temp.begin_node = src;
                    LinkAssignment_single_temp.end_node = dest;
                    LinkAssignment_single_temp.type = 0;
                    LinkAssignment_single_temp.priority = 1;
                    LinkAssignment_single_temp.size = outpredict_real;
        ...
        // mana_slot_traffic
        // slot_traffic_execution_new[SUBNET_NODE_NUMBER][TRAFFIC_SLOT_NUMBER];

        memset(&(macdata_daatr->slot_traffic_execution_new), 0, sizeof(mana_slot_traffic) * SUBNET_NODE_FREQ_NUMBER * TRAFFIC_SLOT_NUMBER);

        // **mana_slot_traffic_new =
        // Generate_mana_slot_traffic(Alloc_slottable_traffic);
        // 将下发给各节点的时隙表进行初始化
            ...
            for (int j = 0; j < TRAFFIC_SLOT_NUMBER; j++)
            {
                if (Alloc_slottable_traffic[j].multiplexing_num != FREQUENCY_DIVISION_MULTIPLEXING_NUMBER)
                {
                    for (int k = 0; k < FREQUENCY_DIVISION_MULTIPLEXING_NUMBER - Alloc_slottable_traffic[j].multiplexing_num; k++)
                    {
                        ...
                        // 然后将其第[j]个时隙的state和send进行修改 修改为 TX , 并将 send
                        // 指向 recv 节点
                        macdata_daatr->slot_traffic_execution_new[Alloc_slottable_traffic[j].send_node[k] - 1][j].state = DAATR_STATUS_TX;
                        macdata_daatr->slot_traffic_execution_new[Alloc_slottable_traffic[j].send_node[k] - 1][j].send_or_recv_node = Alloc_slottable_traffic[j].recv_node[k];
        ...
        // 在收到链路构建需求事件MAC_DAATR_ReceiveTaskInstruct之后 500ms
        // 后发送时隙表
        Message *prepare_msg = MESSAGE_Alloc(node, MAC_LAYER, MAC_PROTOCOL_DAATR, MSG_MAC_ManaPrepareToSendSlottable);
    ...
    case MSG_MAC_ManaPrepareToSendSlottable:
    {
        ...
        {
            // 如果此时节点不在网(转发表为空), 则不向其发送时隙表与跳频图案
            if (macdata_daatr->Forwarding_Table[dest_node - 1][1] == 0)
            {
                continue;
            }

            Message *slottable_msg_1 = MESSAGE_Alloc(node, MAC_LAYER, MAC_PROTOCOL_DAATR, MSG_MAC_SendNewSlottableToEachNode);
            ...
            macdata_daatr->stats.mac_network_overhead += mac_header->packetLength; // 网管节点第一次向各节点发送时隙表

            delete mac_header;

            // 第二次时隙表
            {
                Message *slottable_msg_2 = MESSAGE_Alloc(node, MAC_LAYER, MAC_PROTOCOL_DAATR, MSG_MAC_SendNewSlottableToEachNode);
                ...
                macdata_daatr->stats.mac_network_overhead += mac_header_repeat2->packetLength; // 网管节点第二次发送时隙表
            ...
            // 第三次时隙表
            {
                Message *slottable_msg_3 = MESSAGE_Alloc(node, MAC_LAYER, MAC_PROTOCOL_DAATR, MSG_MAC_SendNewSlottableToEachNode);
                ...
                macdata_daatr->stats.mac_network_overhead += mac_header_repeat3->packetLength; // 第三次发送时隙表
    ...
    case MSG_MAC_ManaPrepareToSendSequence:
    {
        ...
        for (int dest_node = 1; dest_node <= SUBNET_NODE_NUMBER; dest_node++)
        {
            ...
            Message *M_sequence_msg_1 = MESSAGE_Alloc(node, MAC_LAYER, MAC_PROTOCOL_DAATR, MSG_MAC_SendNewSequenceToEachNode);
            ...
            macdata_daatr->stats.mac_network_overhead += mac_header->packetLength; // 网管节点第一次发送跳频图案

            delete mac_header;

            // 2nd
            {
                Message *M_sequence_msg_2 = MESSAGE_Alloc(node, MAC_LAYER, MAC_PROTOCOL_DAATR, MSG_MAC_SendNewSequenceToEachNode);
                MacHeader *mac_header_repeat2 = new MacHeader;
                mac_header_repeat2->is_mac_layer = 2;
                ...
                macdata_daatr->stats.mac_network_overhead += mac_header_repeat2->packetLength; // 第二次跳频图案
            ...
            // 3rd
            {
                Message *M_sequence_msg_3 = MESSAGE_Alloc(node, MAC_LAYER, MAC_PROTOCOL_DAATR, MSG_MAC_SendNewSequenceToEachNode);
                MacHeader *mac_header_repeat3 = new MacHeader;
                mac_header_repeat3->is_mac_layer = 3;
                ...
                macdata_daatr->stats.mac_network_overhead += mac_header_repeat3->packetLength; // 第三次跳频图案
    ...
    case MSG_MAC_SendLocalLinkStateTimer:
    {
        if (macdata_daatr->state_now == Mac_Execution)
        {
            cout << "节点 " << node->nodeId << " 向路由层发送链路状态信息 Time: " << (float)(node->getNodeTime()) / 1000000 << "ms" << endl;
            Send_Local_Link_Status(node, macdata_daatr);
    ...
    { // 定时结束建链阶段, 进入执行阶段
        if (macdata_daatr->has_received_chain_building_request == false && node->nodeId != macdata_daatr->mana_node)
        {
        ...
        else
        {
            ...
            if (macdata_daatr->link_build_state == 0)
            {
                ...
                // MacDaatrInitialize_LocalLinkState_timer(node, macdata_daatr);
                Message *send_msg = MESSAGE_Alloc(node, MAC_LAYER, MAC_PROTOCOL_DAATR, MSG_MAC_SendLocalLinkStateTimer);
                MESSAGE_Send(node, send_msg, 20 * MILLI_SECOND);

                macdata_daatr->link_build_state = 1;
                // 第一次触发(从0到1标志已经建链完毕(包括建链和第一次调整),
                // 可以实现数据包收发)
            }

            MacDaatrInitialize_traffic_Timer(node,
                                             macdata_daatr); // 业务信道定时器初始化
        }
    ...
    case MSG_MAC_SendNewSlottableToEachNode:
    {
        ...
        if (node->nodeId != macdata_daatr->mana_node)
        {
            Message *slottable_ACK_msg = MESSAGE_Alloc(node, MAC_LAYER, MAC_PROTOCOL_DAATR, MSG_MAC_ReceiveACK);
            ...
            macdata_daatr->stats.mac_network_overhead += 25; // 回复给网管节点的ACK
        }

        // 仅对第一次收到时隙表进行打印
        // 并设定计时器进入执行阶段(仅为第一次收到时隙表的情况设定计时器)
        if (macdata_daatr->has_received_slottable == false)
        {
            Message *adjust_state_msg = MESSAGE_Alloc(node, MAC_LAYER, MAC_PROTOCOL_DAATR, MSG_MAC_AllNodesEnterExecutionState);
            ...
            // if (((current_frametime / 1000000) % 1000) < 700) //
            // 第一次收到时隙表的时间
            // {
            //     // 这说明所有节点可以在此1000ms结束前进入执行阶段
            //     delay = (1000 * MILLI_SECOND - current_frametime % (1000 *
            //     MILLI_SECOND));
            // }
            // else
            // {
            //     delay = (2000 * MILLI_SECOND - current_frametime % (1000 *
            //     MILLI_SECOND));
            // }
            cout << endl;
        ...
        // "..\\..\\..\\..\\libraries\\user_models\\src\\Slottable\\Slottable_RXTX_State_"
        // + to_string(static_cast<long long>(node->nodeId)) + ".txt";
        // // cout << stlotable_state_filename << endl;
        // string stlotable_node_filename =
        // "..\\..\\..\\..\\libraries\\user_models\\src\\Slottable\\Slottable_RXTX_Node_"
        // + to_string(static_cast<long long>(node->nodeId)) + ".txt";
        // fout1.open(stlotable_state_filename);
        // fout2.open(stlotable_node_filename);
        ...
        // for (i = 0; i < TRAFFIC_SLOT_NUMBER; i++)
        // {
        //     if (macdata_daatr->slot_traffic_execution[i].state ==
        //     DAATR_STATUS_TX &&
        //     macdata_daatr->slot_traffic_execution[i].send_or_recv_node != 0)
        //     {
        //         fout2 <<
        //         macdata_daatr->slot_traffic_execution[i].send_or_recv_node <<
        //         ",";
        //     }
        //     else if (macdata_daatr->slot_traffic_execution[i].state ==
        //     DAATR_STATUS_RX &&
        //     macdata_daatr->slot_traffic_execution[i].send_or_recv_node != 0)
        //     {
        //         fout2 <<
    ...
    case MSG_MAC_Read_Slottable_From_Txt:
    {
        ...
        // // clocktype delay = (SUBNET_NODE_NUMBER - node->nodeId + 1) * 2 *
        // TRAFFIC_SLOT_DURATION * MILLI_SECOND; clocktype delay =
        // SUBNET_NODE_NUMBER * 2 * TRAFFIC_SLOT_DURATION * MILLI_SECOND;
        // // clocktype delay = 0;
        // cout << endl;
        // cout << "节点 " << node->nodeId << " 读取射前时隙表, Time: " <<
        // (float)(node->getNodeTime()) / 1000000; cout << " 进行初始化定时器操作,
        // 通知进入执行阶段, delay = " << (float)delay / 1000000 << endl;
        // MESSAGE_Send(node, adjust_state_msg, delay);
    ...
    case MSG_MAC_SendNewSequenceToEachNode:
    {
        ...
        if (node->nodeId != macdata_daatr->mana_node)
        {
            Message *sequence_ACK_msg = MESSAGE_Alloc(node, MAC_LAYER, MAC_PROTOCOL_DAATR, MSG_MAC_ReceiveACK);
            Node *dest_node_str = MAPPING_GetNodePtrFromHash(node->partitionData->nodeIdHash, macdata_daatr->mana_node);
    ...
    case MSG_MAC_SendAccessNewSeqence:
    {
        ...
        // MacDaatrInitialize_traffic_Timer(node, macdata_daatr); //
        // 业务信道定时器初始化

        // 暂时不用同步
        // macdata_daatr->if_mana_slottable_synchronization =
        // true;//新接入节点需要作网管信道时隙表同步
    ...
    case MSG_MAC_AllNodesEnterExecutionState:
    {
        ...
        {
            macdata_daatr->stats.slot_adjust_end_time = (float)(node->getNodeTime() / 1000000);
            cout << "------------------------" << endl;
            double adjustment_duration = macdata_daatr->stats.slot_adjust_end_time - macdata_daatr->stats.slot_adjust_begin_time;
            cout << "本次时域调整阶段用时: " << adjustment_duration << "ms" << endl;

            if (node->nodeId == macdata_daatr->mana_node) // 只有网管节点向文件中写入
            {
                macdata_daatr->stats.slot_adjust_num++;
#ifdef ISCAS_LIB
                // Stats_ProSendData(Node *node, int layer, int flag, int type, double
                // value); layer : 3 (MAC) flag : 1 (单子网) type : 17 (时域调整时间)
                // Stats_ProSendData(node, 3, 1, 17, adjustment_duration); //
                // 时域资源调整时间 cout << "已向上层发送 MAC 时域资源调整时间(触发式,
                // 指标17), Time: " << (float)(node->getNodeTime()) / 1000000 << endl;
#endif
            }
        ...
        else if (macdata_daatr->state_now == Mac_Adjustment_Frequency)
        {
            ...
            cout << "------------------------" << endl;
            double adjustment_duration = macdata_daatr->stats.freq_adjust_end_time - macdata_daatr->stats.freq_adjust_begin_time;
            // cout << "本次频域调整阶段用时: " << adjustment_duration << "ms" <<
            // endl;

            if (node->nodeId == macdata_daatr->mana_node) // 只有网管节点向文件中写入
            {
                macdata_daatr->stats.freq_adjust_num++;
#ifdef ISCAS_LIB
                // Stats_ProSendData(Node *node, int layer, int flag, int type, double
                // value); layer : 3 (MAC) flag : 1 (单子网) type : 18 (频域调整时间)
                // Stats_ProSendData(node, 3, 1, 18, adjustment_duration); //
                // 频域调整时间 cout << "已向上层发送 MAC 频域调整时间(触发式,
                // 指标18), Time: " << (float)(node->getNodeTime()) / 1000000 << endl;
#endif
            }
        ...
        if (macdata_daatr->link_build_state == 0)
        {
            Send_Local_Link_Status(node, macdata_daatr);
            MacDaatrInitialize_LocalLinkState_timer(node, macdata_daatr);
            macdata_daatr->link_build_state = 1; // 第一次触发(从0到1标志已经建链完毕(包括建链和第一次调整),
            // 可以实现数据包收发)
            float time = (float)(node->getNodeTime()) / 1000000;
            cout << "Time : " << time << " ms" << endl;
        }
    ...
    case MSG_MAC_NodeDropOutFromSubnet:
    {
        cout << endl;
        cout << "----------------------------" << endl;
        cout << "节点 " << node->nodeId << " 脱网" << endl;
        float time = (float)(node->getNodeTime()) / 1000000;
        cout << "Time : " << time << " ms" << endl;
        cout << "----------------------------" << endl;
        cout << endl;
    ...
    case MSG_MAC_NodeRecoverToSubnet:
    {
        cout << endl;
        cout << "----------------------------" << endl;
        cout << "节点 " << node->nodeId << " 入网" << endl;
        float time = (float)(node->getNodeTime()) / 1000000;
        cout << "Time : " << time << " ms" << endl;
        cout << "----------------------------" << endl;
        cout << endl;
    ...
    case MSG_MAC_Start_Link_Build:
    {
        ...
        if (node->nodeId == macdata_daatr->mana_node) // 提醒网管节点的波束指向
        {
            ...
            if (current_dest_node != 0)
            {
                if (macdata_daatr->state_now == Mac_Initialization)
                {
                    dest_node_ptr = MAPPING_GetNodePtrFromHash(node->partitionData->nodeIdHash,
                                                               current_dest_node); // 获取该接收节点指针
                    ...
                    {
                        Message *build_link_timer_msg = MESSAGE_Alloc(node, MAC_LAYER, MAC_PROTOCOL_DAATR, MSG_MAC_Start_Link_Build);
        ...
        {
            Message *build_link_msg_1 = MESSAGE_Alloc(node, MAC_LAYER, MAC_PROTOCOL_DAATR, MAC_DAATR_LinkBuild_Request);
            Message *build_link_msg_2 = MESSAGE_Alloc(node, MAC_LAYER, MAC_PROTOCOL_DAATR, MAC_DAATR_LinkBuild_Request);
            Message *build_link_msg_3 = MESSAGE_Alloc(node, MAC_LAYER, MAC_PROTOCOL_DAATR, MAC_DAATR_LinkBuild_Request);
            ...
            macdata_daatr->stats.mac_network_overhead += (25 + 2) * 3; // 建链请求，发送三次因此*3

            // cout << "[建链阶段] 节点 " << node->nodeId << "
            // 向网管节点发送建链请求 Time: " << (float)(node->getNodeTime()) /
            // 1000000 << endl;

            Attach_Status_Information_To_Packet(node, dest_node_ptr, macdata_daatr, DAATR_STATUS_TX, 0);
            // Attach_Status_Information_To_Packet(node, dest_node_ptr,
            // macdata_daatr, DAATR_STATUS_TX, 2.5 * MILLI_SECOND);
            // Attach_Status_Information_To_Packet(node, dest_node_ptr,
            // macdata_daatr, DAATR_STATUS_TX, 5 * MILLI_SECOND);

            Attach_Status_Information_To_Packet(node, dest_node_ptr, macdata_daatr, DAATR_STATUS_RX, 12.5 * MILLI_SECOND);
            // Attach_Status_Information_To_Packet(node, dest_node_ptr,
            // macdata_daatr, DAATR_STATUS_RX, 15 * MILLI_SECOND);
            // Attach_Status_Information_To_Packet(node, dest_node_ptr,
            // macdata_daatr, DAATR_STATUS_RX, 17.5 * MILLI_SECOND);
            ...
            {
                Message *build_link_timermsg = MESSAGE_Alloc(node, MAC_LAYER, MAC_PROTOCOL_DAATR, MSG_MAC_Start_Link_Build);
    ...
    case MAC_DAATR_LinkBuild_Request:
    {
        ...
        Message *send_msg0 = MESSAGE_Alloc(node, MAC_LAYER, MAC_PROTOCOL_DAATR, MAC_DAATR_LinkBuild_Response); // 生成消息
        BuildLink_Request build_link;
        ...
        dest_node_ptr = MAPPING_GetNodePtrFromHash(node->partitionData->nodeIdHash, build_link.nodeID); // 获取该接收节点指针
        MESSAGE_Send(dest_node_ptr, send_msg0, 12.5 * MILLI_SECOND);                                    // 相隔5个时隙

        macdata_daatr->stats.mac_network_overhead += (25 + 2); // 网管节点允许建链
    ...
    case MAC_DAATR_SpectrumSensing:
    {
        ...
        for (int i = 0; i < TOTAL_FREQ_POINT; i++)
        {
            // 将接收到的频谱感知结果存储在macdata_daatr结构体中
            macdata_daatr->spectrum_sensing_node[i] = spec_sense_ptr->spectrum_sensing[i];
        }
        ...
        if (node->nodeId == macdata_daatr->mana_node)
        {
            ...
            for (i = 0; i < TOTAL_FREQ_POINT; i++)
            {
                macdata_daatr->spectrum_sensing_sum[macdata_daatr->mana_node - 1][i] = spec_sense_ptr->spectrum_sensing[i];
            }
            macdata_daatr->spectrum_sensing_sum[macdata_daatr->mana_node - 1][TOTAL_FREQ_POINT] = 1;
            ...
            Message *spec_sense_msg = MESSAGE_Alloc(node, MAC_LAYER, MAC_PROTOCOL_DAATR, MAC_DAATR_SpectrumSensing);
            MESSAGE_PacketAlloc(node, spec_sense_msg, sizeof(spectrum_sensing_struct), TRACE_DAATR);
            memcpy((spectrum_sensing_struct *)MESSAGE_ReturnPacket(spec_sense_msg), &spec_struct, sizeof(spectrum_sensing_struct));
            MESSAGE_Send(node, spec_sense_msg, 6000 * MILLI_SECOND);
        }
        else
        {
            // 在业务信道队列中添加此业务
            // 以下是将频谱感知结果转换为01序列, 放在MFC的data字段中,
            // 并将MFC放入队列中等待发送 spectrum_sensing_struct *spec_sen = new
            // spectrum_sensing_struct;
            MacDaatr_struct_converter mac_converter_Spec(5);
            mac_converter_Spec.set_struct((uint8_t *)spec_sense_ptr, 5);
            ...
            cout << "节点 " << node->nodeId << " 将结果存入业务队列中. Time: " << (float)(node->getNodeTime()) / 1000000 << endl;
            MAC_NetworkLayerHasPacketToSend(node, 0,
                                            MFC_temp); // 将此业务添加进业务信道队列
    ...
    case MAC_DAATR_Mana_Node_Recv_Spec_Sensing:
    {
        ...
        MFC_vector_temp = mac_converter_Spec.Get_Single_MFC(); // 将 MFC
        // 转换为01序列的vector以便进行后续的转换
    ...
    // 仅网管节点收到此 event
    case MAC_DAATR_Mana_Node_Recv_Data_Volume:
    {
        uint8_t *bit_seq = (uint8_t *)MESSAGE_ReturnPacket(msg); // 收到的 01 序列
        ...
        MacDaatr_struct_converter mac_converter_DV(99); // MFC(盛放Spec)
        mac_converter_DV - mac_converter;               // mac_converter_Spec 的 seq 中存储 Spec 的 01 序列
        mac_converter_DV.set_length(PDU_TotalLength - 25);

        vector<uint8_t> *MFC_vector_temp = new vector<uint8_t>;
        MFC_vector_temp = mac_converter_DV.Get_Single_MFC(); // 将 MFC
        // 转换为01序列的vector以便进行后续的转换
        msgFromControl *DV_MFC = AnalysisLayer2MsgFromControl(MFC_vector_temp);
        vector<uint8_t> *DV_vector = (vector<uint8_t> *)DV_MFC->data; // 将data字段提取出来, 按照西电的标准是vector

        uint8_t *DV_seq = new uint8_t[4];
        int src_node = DV_MFC->srcAddr;
        int dest_node;
        for (dest_node = 1; dest_node <= SUBNET_NODE_NUMBER; dest_node++)
        {
            for (int i = 0; i < 4; i++)
            {
                DV_seq[i] = (*DV_vector)[4 * (dest_node - 1) + i];
            }
            MacDaatr_struct_converter mac_converter_DV_2(11); // 存放 SpecSensing 的converter
            mac_converter_DV_2.set_bit_sequence(DV_seq, 11);  // 此行为了兼容 type 5 的写法,
            // 将01序列的频谱感知结果赋给converter
            mac_converter_DV_2.daatr_0_1_to_struct(); // 将频谱感知结果的01序列转换为结构体
            Link_DataVolume *DV_struct = (Link_DataVolume *)mac_converter_DV_2.get_struct();

            // 将接收到的链路数据存储为 csv
            if (dest_node != DV_struct->srcAddr)
            {
                string lstm_training_data = "..\\..\\..\\..\\libraries\\user_models\\src\\_LSTM_MAC\\lstm_dataset_per_link\\data_volume_" + to_string(static_cast<long long>(DV_struct->srcAddr)) +
                                            "_" + to_string(static_cast<long long>(dest_node)) + "_" + to_string(static_cast<long long>(COLLECT_DATA_VOLUME_PERIOD)) + ".csv";
                ofstream fout(lstm_training_data, ios::app);

                if (!fout.is_open())
                {
                    cout << "Could Not Open DataVolume File" << endl;
                    system("pause");
                }
                // fout << macdata_daatr->stats.throughput_per_link[dest_node - 1] <<
                // endl;
                fout << DV_struct->data_volume << endl;

                // macdata_daatr->stats.throughput_per_link[dest_node - 1] = 0;

                macdata_daatr->link_data_volume_stats[dest_node - 1].data_volume = 0;
            }
        }
        cout << "网管节点收到来自节点 " << src_node << " 的链路数据信息. Time: " << (float)(node->getNodeTime()) / 1000000 << endl;
    ...
    case MAC_DAATR_Mana_Judge_Enter_Freq_Adjustment_Stage:
    {
        ...
        // float time = (float)(node->getNodeTime()) / 1000000;
        // cout << "Time : " << time << " ms" << endl;
        Judge_If_Enter_Freq_Adjust(node,
                                   macdata_daatr); // 进行判断是否进入频率调整阶段
    ...
    { // 波束维护模块
        // Print_Wave_Info(macdata_daatr->local_node_position_info,
        // macdata_daatr->subnet_other_node_position, node->nodeId);
        // printf("成功维护空域波束信息! \n");

        Message *wave_msg = MESSAGE_Alloc(node, MAC_LAYER, MAC_PROTOCOL_DAATR, MSG_MAC_Beam_maintenance);
        MESSAGE_Send(node, wave_msg, 1000 * MILLI_SECOND);
    ...
    { // 接收来自上层传递的本地飞行状态事件
        ...
        //     TGS_copy_node_topology(macdata_daatr,
        //                            net_neigh->receive_topology); // 将接收到的网络拓扑结构复制到自己的拓扑结构矩阵中
    ...
    case MSG_MAC_LSTM_CollectlinkDataVolume:
    {
        int dest_node;
        if (node->nodeId == macdata_daatr->mana_node)
        {
            for (dest_node = 1; dest_node <= SUBNET_NODE_NUMBER; dest_node++)
            {
                if (dest_node != node->nodeId)
                {
                    string lstm_training_data = "..\\..\\..\\..\\libraries\\user_models\\src\\_LSTM_MAC\\lstm_dataset_per_link\\data_volume_" + to_string(static_cast<long long>(node->nodeId)) + "_" +
                                                to_string(static_cast<long long>(dest_node)) + "_" + to_string(static_cast<long long>(COLLECT_DATA_VOLUME_PERIOD)) + ".csv";
                    ofstream fout(lstm_training_data, ios::app);
                    ...
                    // fout << macdata_daatr->stats.throughput_per_link[dest_node - 1]
                    // << endl;
                    fout << macdata_daatr->link_data_volume_stats[dest_node - 1].data_volume << endl;
        ...
        {
            // 形成UDU放入队列中 与频谱感知结果 MAC_DAATR_SpectrumSensing 类似
            vector<uint8_t> *DV_Data = new vector<uint8_t>;
            for (dest_node = 1; dest_node <= SUBNET_NODE_NUMBER; dest_node++)
            {
                // 生成MFC data字段
                MacDaatr_struct_converter mac_converter_DataVolume(11);
                Link_DataVolume *link_datavolume_temp = new Link_DataVolume;
                link_datavolume_temp->srcAddr = node->nodeId;
                link_datavolume_temp->destAddr = dest_node;
                link_datavolume_temp->data_volume = macdata_daatr->link_data_volume_stats[dest_node - 1].data_volume;

                mac_converter_DataVolume.set_struct((uint8_t *)link_datavolume_temp, 11);
                mac_converter_DataVolume.daatr_struct_to_0_1();
                uint8_t *bit_seq = mac_converter_DataVolume.get_sequence();

                for (int i = 0; i < 4; i++)
                {
                    DV_Data->push_back(bit_seq[i]);
                }
            ...
            MFC_temp->destAddr = macdata_daatr->mana_node; // 给网管节点发送
            MFC_temp->backup = 1;                          // 标识本层MFC数据包
            MFC_temp->data = (char *)DV_Data;
            MFC_temp->priority = 0; // 最高优先级
            MFC_temp->packetLength = 90;
            MFC_temp->msgType = 14;
            MFC_temp->fragmentNum = 14;

            cout << "节点 " << node->nodeId << " 将LSTM数据果存入业务队列中. Time: " << (float)(node->getNodeTime()) / 1000000 << endl;
        ...
        // for (dest_node = 1; dest_node <= SUBNET_NODE_NUMBER; dest_node++)
        // {
        //     cout << macdata_daatr->stats.throughput_per_link[dest_node - 1] << " ";
        // }
        // cout << endl;
        ...
        //     if (dest_node != node->nodeId)
        //     {
        //         string lstm_training_data =
        //             "..\\..\\..\\..\\libraries\\user_models\\src\\_LSTM_MAC\\lstm_dataset_per_link\\data_volume_"
        //             + to_string(static_cast<long long>(node->nodeId)) + "_" +
        //             to_string(static_cast<long long>(dest_node)) + "_" +
        //             to_string(static_cast<long
        //             long>(COLLECT_DATA_VOLUME_PERIOD)) + ".csv";
        //         ofstream fout(lstm_training_data, ios::app);
        ...
        //             system("pause");
        //         }
        //         fout << macdata_daatr->stats.throughput_per_link[dest_node - 1] << endl;
        ...
        // 让当前的网管节点运行一下预测算法 得到最近一秒的预测数据
        // 在 5s 之后再开始预测
        if (node->nodeId == macdata_daatr->mana_node && (float)(node->getNodeTime()) / 1000000 >= 5000)
        {
            LSTM_Generate_predict_data();
        }
    ...
    { // 向上层周期发送子网其他节点位置信息
        ...
        Message *msg4 = MESSAGE_Alloc(node, NETWORK_LAYER, NETWORK_PROTOCOL_NETVIEW, MSG_NETWORK_NETVIEW_ReceiveNetFlightState);
        ...
        memset(macdata_daatr->subnet_other_node_position, 0,
               sizeof(macdata_daatr->subnet_other_node_position)); // 将数组全部设置为0
        cout << node->nodeId << " 向管控层发送其他节点位置信息" << endl;
    ...
    { // 接收身份配置信息
        ...
        if (macdata_daatr->state_now == Mac_Execution)
        {
            ...
            { // 代表此节点脱网
                ...
                macdata_daatr->node_type = Node_Ordinary; // 普通节点
            }
            }
            ...
            { // 如果网管节点发生变化,
                ...
                macdata_daatr->slot_management_other_stage[0].nodeId = mana_node_now;
                macdata_daatr->slot_management_other_stage[26].nodeId = mana_node_now; // 若网管信道时隙表更改，此处更改(25)!!!!!!!!!
                macdata_daatr->slot_management_other_stage[9].nodeId = mana_node_now;
                macdata_daatr->slot_management_other_stage[21].nodeId = mana_node_now;
                macdata_daatr->slot_management_other_stage[34].nodeId = mana_node_now;
                macdata_daatr->slot_management_other_stage[46].nodeId = mana_node_now;
            }
            ...
            cout << "The node " << node->nodeId << " Resonsibility of msgdata is " << temp_nodeResonsibility << endl;
        }
    ...
    case MAC_DAATR_ReceiveForwardingTable:
    {
        ...
        /// cout << "The Routing_table of msgdata is " <<
        /// (*recv_forwarding_table)[1][1] << endl;
        ...
        if (macdata_daatr->state_now == Mac_Execution)
        {
            for (i = 1; i <= SUBNET_NODE_NUMBER; i++)
            {
                macdata_daatr->Forwarding_Table[i - 1][1] = 0;
            ...
            // for (i = 0; i < SUBNET_NODE_NUMBER; i++)
            // {
            //     cout << macdata_daatr->Forwarding_Table[i][0] << "  " <<
            //     macdata_daatr->Forwarding_Table[i][1] << endl;
            // }
        }
        MESSAGE_Free(node, msg);
        break;
    ...
    case MSG_NETWORK_ReceiveNetworkPkt:
    {
        ...
        ////////////////此处接收到网络层vector型0/1消息,
        /// 需要转换为msgfromcontrol结构体类型
        ...
        // cout << "MAC层收到包的头指针地址为" << (int *)&(curPktPayload[0]) <<
        // endl;
        msgFromControl *MFC_temp = new msgFromControl();
        ...
        else // 业务信道消息
        {
            ...
            {
                if (MFC_temp->msgType == 3 && macdata_daatr->Forwarding_Table[MFC_temp->destAddr - 1][1] == 0)
                {
                    cout << node->nodeId << "->" << MFC_temp->destAddr << " 无高速链路, 丢弃数据包 (msgType = 3)" << endl;
                    MESSAGE_Free(node, msg);
                    break;
                }
    ...
    case MSG_MAC_Attach_Information_To_PHY:
    {
        ...
        // {
        //     cout << "{TX} Node {" << node->nodeId << "}  -> " <<
        //     Status_Information->dest_node
        //          << " Time: " << (float)(node->getNodeTime()) / 1000000 << "
        //          ms" << endl;
        // }
        // else
        // {
        //     cout << "{RX} Node {" << node->nodeId << "}  <- " <<
        //     Status_Information->dest_node
        //          << " Time: " << (float)(node->getNodeTime()) / 1000000 << "
        //          ms" << endl;
        // }
        // cout << "天线编号 antennaID: " <<
        // Status_Information->SendOrReceive.antennaID << endl; cout <<
        // "波束方位角 azimuth: " << Status_Information->SendOrReceive.azimuth <<
        // endl; cout << "波束俯仰角 pitchAngle: " <<
        // Status_Information->SendOrReceive.pitchAngle << endl; cout <<
        // "传输速率档位 transmitSpeedLevel: " <<
        // Status_Information->transmitSpeedLevel << endl; cout << "跳频图案
        // frequency_hopping: " << Status_Information->SendOrReceive << endl;
    ...
    { // 物理层接收网管信道参数事件(此事件应放置于物理层中,
      // 目前为测试放置于mac层中)
        Management_Channel_Information *temp_Management_Channel_Information = ((Management_Channel_Information *)MESSAGE_ReturnPacket(msg));
        // cout << "信道收发状态为 " <<
        // temp_Management_Channel_Information->SendOrReceive << endl;
        MESSAGE_Free(node, msg);
        break;
    }
    ...
    case MAC_DAATR_TransParameter:
    {
        ...
        Status_Information_To_Phy *temp_Status_Information_To_Phy = ((Status_Information_To_Phy *)MESSAGE_ReturnPacket(msg));
    ...
    case MSG_MAC_Data_Transmitting:
    {
        ...
        // 考虑到可能在收到这个数据包的时候还没有更新定时器，因此需要提前读取下一个时隙的状态
        // int temp_SlotId;
        // if (macdata_daatr->currentSlotId == 0) // 0~398
        // {
        //     temp_SlotId = TRAFFIC_SLOT_NUMBER - 1;
        // }
        // else
        // {
        //     temp_SlotId = macdata_daatr->currentSlotId - 1;
        // }

        // if (mac_header.srcAddr !=
        // macdata_daatr->slot_traffic_execution[temp_SlotId].send_or_recv_node &&
        //     macdata_daatr->state_now == Mac_Execution)
        // {
        //     cout << node->nodeId << " ";
        //     cout << "本应: " <<
        //     macdata_daatr->slot_traffic_execution[temp_SlotId].send_or_recv_node
        //     << endl;
        // 	cout << "实际: " << mac_header.srcAddr;

        //     cout << "节点 " << node->nodeId << " 与网络内其他节点时隙表不匹配"
        //     << endl; system("pause");
        ...
        for (int i = 0; i < Total_MFC_num; i++)
        {
            ...
            else
            {
                ...
                MAC_NetworkLayerHasPacketToSend(node, interfaceIndex, MFC_temp);
                mfc_count_trans++;
                // cout << node->nodeId << ": " << MFC_temp->srcAddr << "-" <<
                // MFC_temp->destAddr << endl; cout << "MAC 收到非本节点消息
                // 转发对应节点. " << mfc_count_trans << " / " << Total_MFC_num <<
                // endl;
            }
        }
    ...
    case MSG_MAC_PktCounter:
    {
        ...
        // {
        //     cout << "更新前全子网跳频图案干扰比例 " <<
        //     macdata_daatr->stats.subnet_freq_interfer_ratio_before << "%" <<
        //     endl; cout << "更新后全子网跳频图案干扰比例 " <<
        //     macdata_daatr->stats.subnet_freq_interfer_ratio << "%" << endl; for
        //     (int i = 0; i < SUBNET_NODE_FREQ_NUMBER; i++)
        //     {
        //         cout << "更新前节点 " << i + 1 << " 跳频图案干扰比例 " <<
        //         macdata_daatr->stats.node_freq_interfer_ratio_before[i] << "%"
        //         << endl; cout << "更新后节点 " << i + 1 << " 跳频图案干扰比例 "
        //         << macdata_daatr->stats.node_freq_interfer_ratio[i] << "%" <<
        //         endl;
        //     }
        //     for (int i = 0; i < SUBNET_NODE_FREQ_NUMBER; i++)
        //     {
        //         cout << "更新前节点 " << i + 1 << " 窄带干扰比例 " <<
        //         macdata_daatr->stats.node_narrow_band_interfer_ratio_before[i]
        //         << "%" << endl; cout << "更新后节点 " << i + 1 << "
        //         窄带干扰比例 " <<
        //         macdata_daatr->stats.node_narrow_band_interfer_ratio[i] << "%"
        //         << endl;
        //     }
        //     cout << "存在窄带干扰的节点对： " << endl;
        //     vector<int> *temp;
        //     for (int i = 0; i <
        //     macdata_daatr->stats.narrow_band_interfer_nodeID.size(); i++)
        //     {
        //         temp =
        //         &(macdata_daatr->stats.narrow_band_interfer_nodeID.at(i)); cout
        //         << temp->at(0) << "--" << temp->at(1) << "  ";
        //     }
        //     cout << endl;
        // }
        ...
        if (node->nodeId == macdata_daatr->gateway_node || node->nodeId == macdata_daatr->standby_gateway_node)
        {
            cout << endl;
            cout << "本帧内有数据收发的时隙数为 " << macdata_daatr->stats.send_recv_slot_num << endl;
            macdata_daatr->stats.active_slot_ratio = ((float)macdata_daatr->stats.send_recv_slot_num / (TRAFFIC_SLOT_NUMBER * 0.6) * 100);
            cout << "本帧内的活跃时隙比例为 " << macdata_daatr->stats.active_slot_ratio << "%" << endl;
            cout << "----------------------------------------" << endl;
        }
        else

        {
            cout << endl;
            cout << "本帧内有数据收发的时隙数为 " << macdata_daatr->stats.send_recv_slot_num << endl;
            macdata_daatr->stats.active_slot_ratio = ((float)macdata_daatr->stats.send_recv_slot_num / TRAFFIC_SLOT_NUMBER) * 100;
            cout << "本帧内的活跃时隙比例为 " << macdata_daatr->stats.active_slot_ratio << "%" << endl;
            cout << "----------------------------------------" << endl;
        }
        ...
        MESSAGE_Free(node, msg);
        // MacDaatrInitialize_pkt_Counter(node, macdata_daatr); //
        // 此处会清零之前数据!!!!!
...
#ifdef ISCAS_LIB
double Stats_ProGetDataSlotUtil(Node *node, int type)
{
    int interfaceIndex = 0;
    MacDataDaatr *macdata_daatr = (MacDataDaatr *)node->macData[interfaceIndex]->macVar;

    // cout << "上层已获取 MAC 时隙使用比例(周期式, 指标10), Time: " <<
    // (float)(node->getNodeTime()) / 1000000 << endl; cout <<
    // "本帧内有数据收发的时隙数为 " << macdata_daatr->stats.send_recv_slot_num <<
    // endl; cout << "本帧内的活跃时隙比例为 " <<
    // macdata_daatr->stats.active_slot_ratio << "%" << endl;

    int send_recv_slot_num_temp = macdata_daatr->stats.send_recv_slot_num;
    macdata_daatr->stats.send_recv_slot_num = 0;

    return send_recv_slot_num_temp;
}

int Stats_ProGetDataLink(Node *node, int type)
{
    int interfaceIndex = 0;
    MacDataDaatr *macdata_daatr = (MacDataDaatr *)node->macData[interfaceIndex]->macVar;

    int mac_network_overhead_temp = macdata_daatr->stats.mac_network_overhead;

    cout << "上层已获取 MAC 网络开销(周期式, 指标1), Time: " << (float)(node->getNodeTime()) / 1000000 << endl;
    cout << "网络开销: " << macdata_daatr->stats.mac_network_overhead << " Byte" << endl;

    macdata_daatr->stats.mac_network_overhead = 0;

    return mac_network_overhead_temp;
}
#endif
...
void MacDaatrFinalize(Node *node, int interfaceIndex)
{
    ...
    if (node->nodeId == macdata_daatr->mana_node)
    {
        //    Stats_ResSendData(Node *node, int layer, int flag, int type, double
        //    value);

        // Stats_ResSendData(node, 3, 1, 3, macdata_daatr->stats.slot_adjust_num);
        // // 时域资源调整次数 Stats_ResSendData(node, 3, 1, 4,
        // macdata_daatr->stats.freq_adjust_num); // 频域资源调整次数
    }
#endif
}
