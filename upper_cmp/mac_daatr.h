...
#define MAX_WAITING_TIME 1000              // 数据包在队列中的最大等待时间(ms)
...
#define INTERFEREENCE_FREQUENCY_THRESHOLD 0.2 // 干扰频段数量阈值, 若超过此门限, 则进入频率调整阶段
...
// LSTM模块
#define COLLECT_DATA_VOLUME_PERIOD 1000 // 各节点周期性向网管节点上报链路数据量的时间
...
typedef struct LinkAssignment_single
{
    ...
    unsigned short begin_time[3]; // 业务开始时间(每行第一位为时, 取值范围为[1-24], 第二位为分, 取值范围为[0-60],
                                  // 第三位为秒, 取值范围为[0-60]);
    unsigned short end_time[3];   // 业务结束时间(每行第一位为时, 取值范围为[1-24], 第二位为分, 取值范围为[0-60],
                                  // 第三位为秒, 取值范围为[0-60])
...
{
    unsigned short send_node[FULL_CONNECTION_NODE_NUMBER / 2]; // 该时隙分配的发送节点ID, 数目为该子网内的可频分复用数目
    unsigned short recv_node[FULL_CONNECTION_NODE_NUMBER / 2]; // 该时隙分配的接收节点ID, 数目为该子网内的可频分复用数目
...
{
    // 网管信道数据包类型
    uint8_t type; // 0：无定义  1：随遇接入请求  2：同意随遇接入请求回复  3：拒绝接入请求恢复  4：普通节点飞行状态信息
                  // 5：网管节点飞行状态信息  6：网管节点通告消息
} Mana_Packet_Type;
...
// 链路信息统计数据包
typedef struct Link_DataVolume
{
    unsigned int srcAddr;     // 源地址
    unsigned int destAddr;    // 目的地址
    unsigned int data_volume; // src-dest链路的数据量
} Link_DataVolume;
...
typedef struct struct_daatr_stats_str
{
    ...
    uint64_t mac_network_overhead; // 网络开销(单位：byte)
    ...
    int send_recv_slot_num;                      // 本帧内实际收发数据的时隙数
    int throughput_per_link[SUBNET_NODE_NUMBER]; // 统计节点的链路吞吐量(数据量)
    double active_slot_ratio;                    // 本帧内活跃时隙的占比

    int slot_adjust_num; // 时域调整阶段次数
    int freq_adjust_num; // 频域调整阶段次数
...
public:
    ...
    bool daate_link_datavolume_to_0_1();
    bool daate_link_0_1_to_datavolume();
...
typedef struct struct_mac_daatr_str
{
    ...
    bool if_receive_mana_flight_frame[SUBNET_NODE_NUMBER]; // 是否在本帧内收到新的其他节点的飞行状态信息
    ...
    unsigned int subnet_frequency_sequence[TOTAL_FREQ_POINT];                         // 子网所使用频段(共501频点, 对应频点值为1代表使用此频点,
                                                                                      // 0为不使用)(网管节点独有)
    unsigned int spectrum_sensing_node[TOTAL_FREQ_POINT];                             // 本节点频谱感知结果
    unsigned int spectrum_sensing_sum[SUBNET_NODE_FREQ_NUMBER][TOTAL_FREQ_POINT + 1]; // 总频谱感知结果, 只存在于网管节点.最后一列每一行代表一个节点, 若为0,
                                                                                      // 则说明没有接收到此节点的频谱感知结果, 若为1, 则说明接收到了
    ...
    // LSTM
    Link_DataVolume link_data_volume_stats[SUBNET_NODE_FREQ_NUMBER];                                 // 存储20个 固定值
    Link_DataVolume link_data_volume_stats_global[SUBNET_NODE_FREQ_NUMBER][SUBNET_NODE_FREQ_NUMBER]; // 前一个指代第i个节点的链路数据
} MacDataDaatr;
