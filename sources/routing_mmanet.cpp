...
void HandleFormatMsgFromSvc(vector<uint8_t> curDataPacket) {
    vector<msgFromControl *> *msgFromControl_Array =
        GetMsgFromControl(&curDataPacket); // 对接收的格式化消息进行分片，生成多个业务数据包,将其存放在指针指向的数组中
    ...
    // cout << "从上层（服务层）接收格式化消息  destAddr " << (int)(curDataPacket[0] >> 3) << " srcAddr " << (int)((curDataPacket[1] >> 1) & 0x1F) <<
    // endl;
    ...
    cout << "总共发送" << msgFromControl_Array->size() << "个包" << endl;
...
vector<msgFromControl *> *GetMsgFromControl(vector<uint8_t> *dataPacket) {
    ...
    for (size_t i = 1; i <= count; i++) {
        ...
        for (size_t j = 0; j < MMANET_MFC_DATASIZE; j++) {                         // 分片存入数据
            if ((j + (i - 1) * MMANET_MFC_DATASIZE) <= (dataPacket->size() - 1)) { // 若为最后一片，且数据没有存满，空着
...
void HandleMfcFromMac(vector<uint8_t> &curPktPayload) {
    ...
    if (cur_packet_type == 2) { // 消息为路由管理消息
        // cout << "curmsgID->msgsub is " << curmsgID->msgsub << endl;
        switch (curmsgID->msgsub) {
        case 3: {
            if (temp_CRC != cur_msgFromControl->CRC) cout << "temp_CRC != cur_msgFromControl->CRC " << endl;
            if (temp_CRC == cur_msgFromControl->CRC) { // 数据包未出错，接收
                // 收到一个Hello包

                NodeAddress lastSrcAddr = cur_srcAddr; // 记录源节点地址

                clocktype cur_localTimer = ((uint64_t)(curPktPayload)[13] << 56) | ((uint64_t)(curPktPayload)[14] << 48) |
                                           ((uint64_t)(curPktPayload)[15] << 40) | ((uint64_t)(curPktPayload)[16] << 32) |
                                           ((uint64_t)(curPktPayload)[17] << 24) | ((uint64_t)(curPktPayload)[18] << 16) |
                                           ((uint64_t)(curPktPayload)[19] << 8) | ((uint64_t)(curPktPayload)[20]);

                // 计算单项时延
                clocktype curLinkDelay = getCurrentTime() - cur_localTimer;

                // 测试用，将映射关系存入映射表
                NodeAddress cur_Node_ID =
                    ((curPktPayload)[9] << 24) | ((curPktPayload)[10] << 16) | ((curPktPayload)[11] << 8) | ((curPktPayload)[12]);
                ...
                if (mmanet->nodeAddr != cur_srcAddr) {
                    for (int i = 0; i < mmanet->sn_NetNeighList.size(); i++) { // 遍历此时的拓扑表
                        if (mmanet->sn_NetNeighList[i]->nodeAddr == cur_srcAddr) {
                            ...
                            if (cur_msgFromControl->seq > mmanet->sn_NetNeighList[i]->SN ||
                                cur_msgFromControl->seq <= mmanet->sn_NetNeighList[i]->SN - 100 &&
                                    mmanet->sn_NetNeighList[i]->SN > 100) {                  // 若条件成立，代表此片为新
                                for (int j = 0; j < mmanet->BC_msgFromControl.size(); j++) { // 在存放分片的数组中搜寻是否已有
                                    if (mmanet->BC_msgFromControl[j][0]->srcAddr == cur_srcAddr) {
                                        ...
                                        for (int k = 0; k < mmanet->BC_msgFromControl[j].size(); k++) {
                                            if (cur_msgFromControl->fragmentSeq == mmanet->BC_msgFromControl[j][k]->fragmentSeq) {
                                                if (cur_msgFromControl->seq == mmanet->BC_msgFromControl[j][k]->seq) { // 已找到此片
                    ...
                    for (int j = 0; j < mmanet->BC_msgFromControl.size(); j++) {
                        if (mmanet->BC_msgFromControl[j][0]->srcAddr == cur_srcAddr) {
                            ...
                            if (num == cur_msgFromControl->fragmentNum) { // 若已收完此数据的所有片
                                // mmanet->Msg_Flg = true;
                                ...
                                for (int n = 0; n < BC_DataPkt->size() / 14; n++) {
                                    ...
                                    onetemp_NeighborList->Delay =
                                        ((uint64_t)(*BC_DataPkt)[2 + 14 * n] << 56) | ((uint64_t)(*BC_DataPkt)[3 + 14 * n] << 48) |
                                        ((uint64_t)(*BC_DataPkt)[4 + 14 * n] << 40) | ((uint64_t)(*BC_DataPkt)[5 + 14 * n] << 32) |
                                        ((uint64_t)(*BC_DataPkt)[6 + 14 * n] << 24) | ((uint64_t)(*BC_DataPkt)[7 + 14 * n] << 16) |
                                        ((uint64_t)(*BC_DataPkt)[8 + 14 * n] << 8) | ((uint64_t)(*BC_DataPkt)[9 + 14 * n]);
                                ...
                                for (int i = 0; i < mmanet->BC_msgFromControl[j].size();
                                     i++) { // 发送完后清空小于等于该序列号的分片,为了应对边界，增加一个范围
                                    if (mmanet->BC_msgFromControl[j][i]->seq <= cur_msgFromControl->seq ||
                                        cur_msgFromControl->seq <= mmanet->BC_msgFromControl[j][i]->seq - 100 &&
                                            mmanet->BC_msgFromControl[j][i]->seq > 100) {
                                        mmanet->BC_msgFromControl[j].erase(mmanet->BC_msgFromControl[j].begin() + i);
                                        i--;
                                    }
                                }
                                bool Seq_Flg = false;
                                for (int i = 0; i < mmanet->sn_NetNeighList.size(); i++) { // 一个新的结构体去存储sn号，
                                    if (mmanet->sn_NetNeighList[i]->nodeAddr == cur_srcAddr) {
                                        Seq_Flg = true;
        ...
        case 6: {
            ...
            if (temp_CRC == cur_msgFromControl->CRC) {
                ...
                bool fragmentSeq_flg = false;
                if (mmanet->nodeAddr != cur_srcAddr) {
                    for (int i = 0; i < mmanet->disu_sn_NetNeighList.size(); i++) { // 遍历此时的拓扑表
                        if (mmanet->disu_sn_NetNeighList[i]->nodeAddr == cur_srcAddr) {
                            ...
                            if (cur_msgFromControl->seq > mmanet->disu_sn_NetNeighList[i]->SN ||
                                cur_msgFromControl->seq <= mmanet->disu_sn_NetNeighList[i]->SN - 100 &&
                                    mmanet->disu_sn_NetNeighList[i]->SN > 100) {              // 若条件成立，代表此片为新
                                for (int j = 0; j < mmanet->BC_msgFromControl2.size(); j++) { // 在存放分片的数组中搜寻是否已有
                                    if (mmanet->BC_msgFromControl2[j][0]->srcAddr == cur_srcAddr) {
                                        ...
                                        for (int k = 0; k < mmanet->BC_msgFromControl2[j].size(); k++) {
                                            if (cur_msgFromControl->fragmentSeq == mmanet->BC_msgFromControl2[j][k]->fragmentSeq) {
                                                if (cur_msgFromControl->seq == mmanet->BC_msgFromControl2[j][k]->seq) { // 已找到此片
                                                    fragmentSeq_flg = true;
                                                }
                                            }
                                        }
                    ...
                    for (int j = 0; j < mmanet->BC_msgFromControl2.size(); j++) {
                        if (mmanet->BC_msgFromControl2[j][0]->srcAddr == cur_srcAddr) {
                            ...
                            if (num == cur_msgFromControl->fragmentNum) { // 若已收完此数据的所有片
                                ...
                                for (int i = 0; i < mmanet->BC_msgFromControl2[j].size();
                                     i++) { // 发送完后清空小于等于该序列号的分片,为了应对边界，增加一个范围
                                    if (mmanet->BC_msgFromControl2[j][i]->seq <= cur_msgFromControl->seq ||
                                        cur_msgFromControl->seq <= mmanet->BC_msgFromControl2[j][i]->seq - 100 &&
                                            mmanet->BC_msgFromControl2[j][i]->seq > 100) {
                                        mmanet->BC_msgFromControl2[j].erase(mmanet->BC_msgFromControl2[j].begin() + i);
                                        i--;
                                    }
                                }
                                bool Seq_Flg = false;
                                for (int i = 0; i < mmanet->disu_sn_NetNeighList.size(); i++) { // 一个新的结构体去存储sn号，
                                    if (mmanet->disu_sn_NetNeighList[i]->nodeAddr == cur_srcAddr) {
                                        Seq_Flg = true;
        ...
        // break;
    } else if (cur_packet_type == 3) {                       // 消息为业务消息
        if (temp_CRC == cur_msgFromControl->CRC) {           // 数据包未出错，接收
            if (!mmanet->ToNetwork_msgFromControl.empty()) { // 之前已经接收过消息分片
                // 遍历其源地址和序列号来寻找属于哪一个格式化消息的片段
                for (size_t i = 0; i < mmanet->ToNetwork_msgFromControl.size(); i++) {
                    if ((mmanet->ToNetwork_msgFromControl[i][0]->srcAddr == cur_msgFromControl->srcAddr) &&
                        (mmanet->ToNetwork_msgFromControl[i][0]->seq == cur_msgFromControl->seq)) {
                        // 源地址和序列号都相同，找到该分片是属于哪一个格式化消息的片段，再遍历判断该分片是否已经被接收
                        for (size_t j = 0; j < mmanet->ToNetwork_msgFromControl[i].size(); j++) {
                            // 若该分片已经被接收，则丢弃
                            if (cur_msgFromControl->fragmentSeq == mmanet->ToNetwork_msgFromControl[i][j]->fragmentSeq) break;

                            if (j == mmanet->ToNetwork_msgFromControl[i].size() - 1) { // 遍历完了，仍然没有。即该分片是新分片，接收
                                ...
                                if ((mmanet->ToNetwork_msgFromControl[i].size() == mmanet->ToNetwork_msgFromControl[i][0]->fragmentNum) &&
                                    mmanet->FormatMessageControl[i].reorganizationCompleted == false) {
                                    // 所有分片已经接收，调用格式化消息重组发送函数进行重组并发送给上层
                                    cout << "所有格式化消息分片已经接收，调用格式化消息重组发送函数进行重组并发送给上层" << endl;
                    ...
                    if (i == mmanet->ToNetwork_msgFromControl.size() - 1) { // 遍历完了，表明新分片是属于一条新的格式化消息，接收
                        // 创建一条新的消息队列
                ...
                for (size_t m = 0; m < mmanet->FormatMessageControl.size(); m++) {
                    if (((getCurrentTime() - mmanet->FormatMessageControl[m].lastMsgFromControlTimer) > MMANET_TOUPPERMSG_LIFECYCLE) &&
                        mmanet->FormatMessageControl[m].reorganizationCompleted == true) {
    ...
    } else if (cur_packet_type == 1 || cur_packet_type == 4) { // 消息为链路层传给网管模块的消息，直接透传给网管模块
...
// 按照分片序号将业务数据包进行升序排列
bool CMP(msgFromControl *a, msgFromControl *b) { return a->fragmentSeq < b->fragmentSeq; }
...
void timer_callback(int signum, siginfo_t *si, void *uc) {
    ...
    if (si->si_value.sival_int == 1) {
        ...
        // for (int ik = 0; ik < mmanet->NetNeighList.size(); ik++)
        // {
        // 	cout << mmanet->NetNeighList[ik]->nodeAddr << "  size(): " << mmanet->NetNeighList[ik]->localNeighList.size() << " node ";
        // 	for (auto node : mmanet->NetNeighList[ik]->localNeighList)
        // 	{
        // 		cout << " " << node->nodeAddr;
        // 	}
        // 	cout << endl;
...
void Count_List() {
    ...
    for (size_t i = 0; i < mmanet->LFT_Entry.size(); i++) {
        if (mmanet->LFT_Entry[i]->hop > 100) { // 100是随便给的一个数，表示跳数大于100跳，不可达
...
void FillNodeAddrToIndexTable() {
    ...
    for (size_t i = 0; i < mmanet->NetNeighList.size(); i++) {
        ...
        for (size_t j = 0; j < mmanet->NetNeighList[i]->localNeighList.size(); j++) {
            vector<uint16_t>::iterator iter2 =
                find(mmanet->NodeAddrToIndex.begin(), mmanet->NodeAddrToIndex.end(), (mmanet->NetNeighList[i]->localNeighList)[j]->nodeAddr);
...
int ReturnSNinLocalNeighMatrix(uint16_t nodeAddr) {
    for (size_t i = 0; i < mmanet->NodeAddrToIndex.size(); i++) {
        if (mmanet->NodeAddrToIndex[i] == nodeAddr) return i;
    }
...
int ReturnAddrinLocalNeighMatrix(size_t coor) {
    if (coor >= mmanet->NodeAddrToIndex.size()) return -1; // 超出范围，返回-1表明数据错误
...
void DijkstraAlgorithm() {
    ...
    // 拷贝构造
    vector<vector<nodeNetForwardingTable *>> NewNetRT_Entry =
        vector<vector<nodeNetForwardingTable *>>(mmanet->NodeAddrToIndex.size(), vector<nodeNetForwardingTable *>(mmanet->NodeAddrToIndex.size(), 0));
    ...
    for (size_t coor = 0; coor < mmanet->NodeAddrToIndex.size(); coor++) {
        ...
        for (i = 0; i < mmanet->NodeAddrToIndex.size(); i++) {
            ...
            for (j = 0; j < mmanet->NodeAddrToIndex.size(); j++) {
                ...
                if (flag[j] == 0 && (tmp < newRTM->dist[j])) { // 若节点j还未获得最短路径且从节点k到节点j得到的距离比表格记录的距离短
        ...
        for (i = 0; i < mmanet->NodeAddrToIndex.size(); i++) {
            ...
            if (i != coor) {
                if (nowCoor == coor) FillLocalForwardingTable(newRTM, i);
            }
    ...
    // {
    // 	cout << mmanet->LFT_Entry[ji]->destAddr << " " << mmanet->LFT_Entry[ji]->nextHop << " " << mmanet->LFT_Entry[ji]->hop << " " <<
    // mmanet->LFT_Entry[ji]->delay << endl;
    // }
    // cout << "全网路由表表项：" << endl;
    ...
    // 将低速一跳可达的路径加入路由表中
    if (mmanet->NetRT_Entry.size() == 0) { // 为空则表示所有点都高速不可达，直接将低速一条可达的路径加入路由表中
            ...
            for (size_t i = 0; i < mmanet->disu_NetNeighList.size(); i++) {
                for (size_t j = 0; j < mmanet->disu_NetNeighList[i]->localNeighList.size(); j++) {
                    for (size_t m = 0; m < mmanet->NetRT_Entry.size(); m++) {
                        if ((mmanet->disu_NetNeighList[i]->localNeighList)[j]->nodeAddr == mmanet->NetRT_Entry[m][0]->source) break;
...
void FillLocalForwardingTable(routingTableMessage *newRTM, int dest_coor) {
    ...
    // {
    // 	// cout << mmanet->LFT_Entry[i]->destAddr << " " << mmanet->LFT_Entry[i]->nextHop << " " << mmanet->LFT_Entry[i]->hop << " " <<
    // mmanet->LFT_Entry[i]->delay << endl;
    // }
}
...
bool CMP_NetRT_Entry(nodeNetForwardingTable *a, nodeNetForwardingTable *b) { return a->dest < b->dest; }
...
void SendForwardingTable() {
    ...
    printTime_ms();
    cout << "全网路由表: " << endl;
    cout << "source  dest  hop  nexthop " << endl;
    // 使用深拷贝，将里面存放的指针也new一遍
    NetRT_EntryMsg *Cur_RT_Entry = new NetRT_EntryMsg();
    Cur_RT_Entry->netRTtablePtr =
        new vector<vector<nodeNetForwardingTable *>>(mmanet->NetRT_Entry.size(), vector<nodeNetForwardingTable *>(mmanet->NetRT_Entry.size(), 0));
    for (size_t i = 0; i < mmanet->NetRT_Entry.size(); i++) {
        for (size_t j = 0; j < mmanet->NetRT_Entry[i].size(); j++) {
            ...
            cout << toNetForwardingTable->source << " " << toNetForwardingTable->dest << " " << toNetForwardingTable->hop << " "
                 << toNetForwardingTable->nexthop << endl;
        }
    }
...
void SendTopologyTable() {
    ...
    for (size_t i = 0; i < mmanet->NetNeighList.size(); i++) {
        ...
        cout << "nodeAddr: " << toNetTopologyTable->nodeAddr << " ,neighbor: ";
        for (size_t j = 0; j < (mmanet->NetNeighList[i]->localNeighList).size(); j++) {
            ...
            toNetTopologyTable->localNeighList.push_back(toNetLocalNeighList);
            cout << toNetLocalNeighList->nodeAddr << " ";
        }
        ...
        if (mmanet->NetNeighList[i]->nodeAddr == newNetNeighListMember->nodeAddr) {
            for (size_t j = 0; j < mmanet->NetNeighList[i]->localNeighList.size(); j++) {
                for (size_t k = 0; k < newNetNeighListMember->localNeighList.size(); k++) {
                    if (mmanet->NetNeighList[i]->localNeighList[j]->nodeAddr == newNetNeighListMember->localNeighList[k]->nodeAddr) break;
            ...
            for (size_t j = 0; j < newNetNeighListMember->localNeighList.size(); j++) {
                for (size_t k = 0; k < mmanet->NetNeighList[i]->localNeighList.size(); k++) {
                    if (newNetNeighListMember->localNeighList[j]->nodeAddr == mmanet->NetNeighList[i]->localNeighList[k]->nodeAddr) break;
                    ...
                    if (k == mmanet->NetNeighList[i]->localNeighList.size() - 1) {
                        ...
                        for (size_t m = 0; m < mmanet->NetNeighList.size(); m++) {
                            if (newNetNeighListMember->localNeighList[j]->nodeAddr == mmanet->NetNeighList[m]->nodeAddr) {
                                for (size_t n = 0; n < mmanet->NetNeighList[m]->localNeighList.size(); n++) {
                                    if (mmanet->NetNeighList[m]->localNeighList[n]->nodeAddr == newNetNeighListMember->nodeAddr) break;
                                    ...
                                    if (n == mmanet->NetNeighList[m]->localNeighList.size() - 1) {
                                        ...
                                        sort(mmanet->NetNeighList[m]->localNeighList.begin(), mmanet->NetNeighList[m]->localNeighList.end(),
                                             CMP_neighborList);
                                        break;
                                    }
                                }
...
// 按照节点序号将本地邻居表进行升序排列
bool CMP_neighborList(nodeLocalNeighList *a, nodeLocalNeighList *b) { return a->nodeAddr < b->nodeAddr; }
...
// 		cout << "链路层接收链路状态信息neighborList: " << Cur_neighborList.size() << " node:  ";
// 		for (auto node : Cur_neighborList)
// 		{
// 			cout << " " << node.nodeAddr;
// 		}
// 		cout << endl;
...
void MSG_ROUTING_ReceiveLocalLinkMsg(vector<uint8_t> &curPktPayload, uint16_t len) {
    ...
    if (mmanet->Hello_Ack_Flg == false) {
        ...
        Cur_neighborList = ReceiveLocalLinkState->neighborList;
        // cout << "len " << len << endl;
        // cout << "ReceiveLocalLinkState_Msg .neiborsize() " << Cur_neighborList.size() << " ,node are:";
        // for (auto node : Cur_neighborList)
        // {
        // 	cout << node.nodeAddr << " ";
        // }
        ...
        for (const auto &newNeighbor : Cur_neighborList) {
            auto it = find_if(mmanet->old_neighborList.begin(), mmanet->old_neighborList.end(),
                              [&newNeighbor](const nodeLocalNeighList *node) { return node->nodeAddr == newNeighbor.nodeAddr; });
            if (it != mmanet->old_neighborList.end()) {
                mmanet->old_neighborList.erase(it);
            }
            mmanet->old_neighborList.push_back(new nodeLocalNeighList(newNeighbor));
            // cout << " " << newNeighbor.nodeAddr;
        }
        mmanet->cur_neighborList.clear();
        mmanet->cur_neighborList = mmanet->old_neighborList;
        // 清空 old_neighborList，删除其中的元素,将mmanet->cur_neighborList给mmanet->old_neighborList
        // if (mmanet->old_neighborList.size() != 0)
        // {
        // 	for (auto node : mmanet->old_neighborList)
        // 	{
        // 		delete node;
        ...
        // }
        mmanet->old_neighborList.clear();
        mmanet->old_neighborList = mmanet->cur_neighborList;
        // mmanet->cur_neighborList.clear();

        // 更新 cur_neighborList
        // cout << " mmanet->cur_neighborList 添加邻居 : ";
        // if (mmanet->cur_neighborList.size() != 0)
        // {
        // 	for (auto node : mmanet->cur_neighborList)
        // 	{
        // 		delete node;
        ...
        // 	}
        // }
        // cout << "Routing开始MMANETBroadcastHello" << endl;
        MMANETBroadcastHello();
        // delete &curPktPayload;
    }
}
...
bool CMP_NetNeighList(nodeNetNeighList *a, nodeNetNeighList *b) { return a->nodeAddr < b->nodeAddr; }
...
void MMANETBroadcastMessage(char *NeighList, int a) {
    ...
    for (int i = 1; i <= count; i++) {
        ...
        cur_data->push_back((curmsgID->msgid << 3) | (curmsgID->msgsub));
        for (int j = 0; j < BC_datasize; j++) { // 分片存入数据
