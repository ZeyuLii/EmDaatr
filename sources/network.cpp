...
void *NetReceiveFromMac(void *arg) {
    ...
    while (!end_simulation) {
        ...
        case 0x18: // 身份配置信息发送指示
            ...
            if (nodeNotificationPtr->nodeResponsibility == 1) ControlNodeDealWithIdentityStatus(dataPacket_svc);
            break;
...
uint64_t analysisPtr(const vector<uint8_t> &msgPtr) {
    if (msgPtr.size() == 8) {
        uint64_t ptr = ((uint64_t)msgPtr[7] << 56) | ((uint64_t)msgPtr[6] << 48) | ((uint64_t)msgPtr[5] << 40) | ((uint64_t)msgPtr[4] << 32) |
                       ((uint64_t)msgPtr[3] << 24) | ((uint64_t)msgPtr[2] << 16) | ((uint64_t)msgPtr[1] << 8) | ((uint64_t)msgPtr[0]);
    } else {
...
void LinkConfigInit(LinkConfigData *linkConfigPtr) {
    // 节点id如何获取？
    linkConfigPtr->nodeId = mmanet->nodeAddr;
    // 判断本地节点的身份
    if (nodeNotificationPtr->nodeResponsibility == INTRAGROUPCONTROL) {
        ...
        cout << "控制节点设置周期性下发给链路层" << endl;
        SetTimerUser2(14, 999, 6, NULL, 0); // 6为定时器编号
    } else {
        // 其余为普通节点，设置周期性转发给控制节点
        cout << "其余为普通节点，设置周期性转发给控制节点" << endl;
    ...
    if (rTtablePtr) {
        ...
        for (i = rTtablePtr->begin(); i != rTtablePtr->end(); i++) {
            ...
            for (j = i->begin(); j != i->end(); j++) {
                ...
                // // printf("src:%u dest:%u \n",(*j)->source, (*j)->dest);
                cout << "src: " << (*j)->source << " dest: " << (*j)->dest << " hop: " << (*j)->hop << " nexthop: " << (*j)->nexthop
                     << " delay: " << (*j)->delay << endl;
                cout << "Relaynodes is:  ";
...
void HandleReceivedTaskInstruct(const vector<uint8_t> &task_MsgPtr) {
    ...
    if (nodeNotificationPtr->intragroupcontrolNodeId) {
        ...
        // cout << "linkConfigPtr->linkAllocList.size()  " << linkConfigPtr->linkAllocList.size() << endl;
        if (nodeNotificationPtr->nodeResponsibility != MASTERCONTROL && nodeNotificationPtr->nodeResponsibility != INTRAGROUPCONTROL &&
            ...
            linkConfigPtr->linkAllocList.size() >= 1) {
            // cout << "自己不是控制节点" << endl;
    ...
    for (int i = 0; i < fullLinkNum; i++) {
        ...
        if (resultLinkPtr) {
            ...
            if (resultLinkPtr->listNumber < BUSSINE_TYPE_NUM) {
                ...
                cout << "非空，找到了 " << resultLinkPtr->size[resultLinkPtr->listNumber] << endl;
            ...
            // auto it = linkConfigPtr->linkAllocList.begin();
            // cout << "BreakFullRoute linkConfigPtr->linkAllocList.size()  " << linkConfigPtr->linkAllocList.size() << " (*it)->size[0] " <<
            // (*it)->size[0] << "Done" << endl;
        }
...
void PrintLinkAssignment(LinkAssignment *taskPtr) {
    ...
    for (int i = 0; i < listNum; i++) {
        printf("Begin: %u, end: %u, type: %u, pri: %u, size: %.4f QOS: %u ", taskPtr->begin_node, taskPtr->end_node, taskPtr->type[i],
               taskPtr->priority[i], taskPtr->size[i], taskPtr->QOS[i]);
...
vector<uint8_t> *LinkConfigAddHeader_controlmsg(NodeAddress destId, char *pktStart, int payLoadSize, int moduleName, int eventType) {
    ...
    // cout << "节点 " << linkConfigPtr->nodeId << " 向节点 " << destId << "汇总链路构建需求， 开销为 " << v1_controlmsg->size() + 25 << "目前总开销为
...
void CheckTaskListTimeout() {
    cout << "定期检查自己的任务列表,是否发送链路构建需求" << endl;
    unsigned int actualLinkNum = linkConfigPtr->linkAllocList.size();
    if (actualLinkNum) {
        // 不为0，有链路才进行发送
        unsigned int payLoadSize = sizeof(LinkAssignmentHeader) + actualLinkNum * sizeof(LinkAssignment);
        ...
        if (nodeNotificationPtr->nodeResponsibility == MASTERCONTROL || nodeNotificationPtr->nodeResponsibility == INTRAGROUPCONTROL) {
            // 如果是总/分控制节点，给链路层发
            //**************新增链路运行状态判断*****************************
            bool flag = canSendConfigLinkRequest();
            if (!flag) {
                // 如果不是任务执行阶段
        ...
        if (nodeNotificationPtr->nodeResponsibility == MASTERCONTROL || nodeNotificationPtr->nodeResponsibility == INTRAGROUPCONTROL) {
            // 控制节点下发链路构建需求周期为10s
            // cout << "控制节点下发链路构建需求周期为10s" << endl;
            SetTimerUser2(10, 0, 6, NULL, 0); // 6为定时器编号
        } else {
            // 普通节点汇聚链路构建需求周期为5s
            // cout << "普通节点汇聚链路构建需求周期为5s" << endl;
...
void HandleControlMsgFromRoute(const vector<uint8_t> &control_MsgPtr) {
    ...
    if (crc == controlPtr->CRC) // CRC校验通过，做分流操作
    {
        ...
        if (eventType == 0) {
            ...
            // 其他节点处理控制节点发来的身份广播包
            cout << "其他节点处理控制节点通过路由模块传来发来的身份广播包" << endl;
            ...
            if (resultLinkPtr) {
                ...
                if (resultLinkPtr->listNumber < BUSSINE_TYPE_NUM) {
                    for (int j = 0; j < linkPtr[i].listNumber; j++) {
                        ...
                        resultLinkPtr->listNumber++;
                        if (resultLinkPtr->listNumber >= BUSSINE_TYPE_NUM) break;
                    }
                }
            } else {
                // 没有相同的，需新建
...
char *AnalysisDataInMsgFromControl(vector<uint8_t> *dataPacket) {
    ...
    for (int i = 0; i < linkheader->linkNum; i++) {
        ...
        for (int j = 0; j < linkAssPtr[i].listNumber; j++) {
            ...
            linkAssPtr[i].size[j] = ((*dataPacket)[cnt + 10 + j * 24 + servicesNum] << 24) | ((*dataPacket)[cnt + 11 + j * 24 + servicesNum] << 16) |
                                    ((*dataPacket)[cnt + 12 + j * 24 + servicesNum] << 8) | (*dataPacket)[cnt + 13 + j * 24 + servicesNum];
...
// 所有邻居，按id排序
bool sortNeighborByNodeId(LinkQuality *neighbor1, LinkQuality *neighbor2) { return neighbor1->nodeId < neighbor2->nodeId; }
...
void HandleReceivedNetPos(const vector<uint8_t> &NetPosMsgPtr) {
    ...
    // printf(" nodePosMsg nodenum is %d\n", nodePosMsg->nodeNum);
    ...
    // {
    //     cout << "nodeid = " << nodePosList[i].nodeId << " ,X " << nodePosList[i].positionX << " ,Y " << nodePosList[i].positionY << " ,Z " <<
    //     nodePosList[i].positionZ << endl;
    // }
    ...
    if (netViewPtr->netViewList.size() != 0) {
        ...
        if (nodeNotificationPtr->nodeResponsibility == INTRAGROUPCONTROL || nodeNotificationPtr->nodeResponsibility == MASTERCONTROL) {
            // SpacebasedUpdate();
            cout << "进行节点身份正常更新，只在控制节点上进行 " << endl;
            IntraGroupUpdate();
        }
    } else {
...
void ReadNetPos() {
    ...
    for (i = 0; i < nodeNum; i++) {
        ...
        if (findNode) {
            ...
            // cout << " findNode->thisPos.positionX " << findNode->thisPos.positionX << " findNode->thisPos.positionY " <<
            // findNode->thisPos.positionY << " findNode->thisPos.positionZ " << findNode->thisPos.positionZ << endl; 写入位置//
            ...
            // printf("Not find node %u posX: %f posY: %f posZ: %f\n", nodeId, nodePosList[i].positionX, nodePosList[i].positionY,
            // nodePosList[i].positionZ);
        }
    }
...
void HandleLocalFlightState(const vector<uint8_t> &LocalFlightStateMsgPtr) {
    ...
    // cout << "节点 " << flightToTrans.nodeId << " 收到飞行状态，位置为 " << flightToTrans.positionX << " " << flightToTrans.positionY << " " <<
    // flightToTrans.positionZ << endl;
...
void HandleIdentityUpdate(NodeNotification *nodeReponseMsgPtr) {
    if (netViewPtr->nodeId == nodeReponseMsgPtr->intragroupcontrolNodeId) // 判断是自己是控制节点
    {
        if (netViewPtr->netViewList.size() != 0) // 网络状态视图不为空
        {
            ...
            cout << "理身份配置模块传来的身份配置信息，网络状态视图为空" << endl;
            // 等待一段时间重新触发
...
void IntraGroupControlUpdate(list<NodeCondition *> *ptr) {
    ...
    for (myitr = ptr->begin(); myitr != ptr->end(); ++myitr) {
        ...
        UpdateFactorTmp.nodePriorityFactor =
            evaluatePriorityFactor(UpdateFactorTmp.nodeDistance, UpdateFactorTmp.nodeSpeed, UpdateFactorTmp.nodePayLoad, maxDistance);
        listUpdateFactor.push_back(UpdateFactorTmp);
    } // 计算更新因子
    listUpdateFactor.sort(sortByNodePriorityFactor); // 更新因子排序

    cout << "更新因子排序 : ";
    for (auto e : listUpdateFactor) {
        cout << " " << e.nodeId;
    }
    cout << endl;
    ...
    for (list<NodeCondition *>::iterator it = ptr->begin(); it != ptr->end(); it++) {
        if (UfTmp.nodeId == (*it)->nodeId) {
            // cout << "获取本轮待定备份的节点 " << (*it)->nodeId << endl;
...
void HandleIdentityBroadcastTiming() {
    ...
    if (nodeNotificationPtr->nodeResponsibility == INTRAGROUPCONTROL ||
        ...
        if (IdentityPtr->If_calculated) {
            cout << "是总控制或群内控制,进行了身份计算 " << endl;
            writeInfo("触发定时下发身份广播包的函数");
            // 发广播包函数
...
bool IsIdentityUpdate(NodeBroadcast *contentPtr) {
    ...
    // 其他关键节点身份待补充
    if (contentPtr->intragroupcontrolNodeId == nodeNotificationPtr->intragroupcontrolNodeId &&
        contentPtr->reserveintracontrolNodeId == nodeNotificationPtr->reserveintracontrolNodeId) {
        flag = false;
    }
    return flag;
}
...
void ControlNodeDealWithIdentityStatus(const vector<uint8_t> &IdentityStatusMsgPtr) {
    ...
    if (identityStatus_Msg->isBroadcast && identityStatus_Msg->seq == IdentityPtr->SerialNum) // 广播成功 且 传回序列号是否与本轮待广播序列号相同
    {
        ...
        nodeBroadcastPtr->satellitebasedgatewayNodeId = IdentityPtr->satellitebasedgateway_Temp;               // 天基
        nodeBroadcastPtr->reservesatellitebasedgatewayNodeId = IdentityPtr->reservesatellitebasedgateway_Temp; // 天基备份
        IdentityPtr->IsBroadcast = true;
        // cout << "IdentityPtr->IsBroadcast = true;" << endl; // 更新广播成功标志位

        // cout << "收到0x18,打印 " << endl;
        // cout << "groupID: " << nodeBroadcastPtr->groupID << endl;
        // cout << "intragroupcontrolNodeId: " << nodeBroadcastPtr->intragroupcontrolNodeId << endl;
        // cout << "reserverdintracontrolNodeId: " << nodeBroadcastPtr->reserveintracontrolNodeId << endl;

        // 10ms后，调用ReceiveAMsgFromOtherNodes函数，使控制节点处理自身打包的身份广播包
        // 触发这个函数说明链路层已经成功将身份包广播出去了，此时控制节点需要等待一个经验性的广播时延，再更新自己的身份表
        // cout << "链路层已经成功将身份包广播出去了，此时控制节点需要等待一个经验性的广播时延，再更新自己的身份表 " << endl;
        // cout << "nodeBroadcastPtr: " << nodeBroadcastPtr << endl;
        // cout << "&nodeBroadcastPtr: " << &nodeBroadcastPtr << endl;
        // SetTimerUser2(0, 10, 9, nodeBroadcastPtr, sizeof(nodeBroadcastPtr));
        usleep(10000);
        ReceiveAMsgFromOtherNodes((char *)nodeBroadcastPtr);
...
void InformIdentityToOtherModules() {
    ...
    if (flag) {
        ...
        cout << "节点开始通知其他模块身份链路层和路由模块） " << endl;
        // 发送给链路层
        uint8_t wBufferToMac[MAX_DATA_LEN];                                            // 将要写入循环缓冲区的数据(含包头)存放于此
        PackRingBuffer(wBufferToMac, nodeIdentityPtr, sizeof(NodeNotification), 0x0A); // 身份配置信息
        // MAC_Daatr_Receive->ringBuffer_put(wBufferToMac, sizeof(wBufferToMac));
        ...
        // 过500ms重新调用该函数
        cout << "通知其他模块（链路层和路由模块）身份配置信息,过500ms重新调用该函数 " << endl;
...
void ReceiveDamagedNodesmsg(const vector<uint8_t> &DamagedNodeMsgPtr) {
    ...
    if (DamagedPtr->DamagedNodes != nullptr) {
        ...
        for (myitr = damagedAllNodes.begin(); myitr != damagedAllNodes.end(); myitr++) {
            ...
            if ((*myitr) == nodeNotificationPtr->intragroupcontrolNodeId || (*myitr) == nodeNotificationPtr->mastercontrolNodeId ||
        ...
        for (myitr = damagedAllNodesPtr->begin(); myitr != damagedAllNodesPtr->end(); myitr++) {
            ...
            if ((*myitr) == nodeNotificationPtr->intragroupcontrolNodeId) {
                ...
                if (IdentityPtr->nodeId == destId) {
                    ...
                    netToRouting_buffer.ringBuffer_put(wBufferToRouting, sizeof(wBufferToRouting));
                    cout << "处理路由模块传来的毁伤信息且0x1E给routting" << endl;
                }
...
vector<uint8_t> *MsgFromControl_ConvertDamage(NodeAddress destId, void *data, int moduleName, int eventType) {
    ...
    for (int i = 0; i < damaged_Num; i++) {
        DamagedNode *damagedNodePtr =
            (DamagedNode *)(msgToSendPtr + sizeof(msgID) + sizeof(MessageDiversionHeader) + sizeof(DamagedHeader) + i * sizeof(DamagedNode));
    ...
    vector<uint8_t> *data_control = (vector<uint8_t> *)controlmsg.data;
    unsigned int eventType1 = ((*data_control)[3] << 8) | (*data_control)[4];
...
void ControlNodeDealWithDamage(msgFromControl *controlMsgPtr) {
    ...
    for (int i = 0; i < damagedHeaderPtr->damagedNum; i++) {
        DamagedNode *damagedNodePtr =
            (DamagedNode *)(payloadStart + sizeof(msgID) + sizeof(MessageDiversionHeader) + sizeof(DamagedHeader) + i * sizeof(DamagedNode));
        // 查询是否在已处理毁伤列表中
...
void HandleDamageMsg(NodeAddress damaged_node) {
    ...
    // 统一处理其他节点毁伤
    if (IdentityPtr->nodeId == nodeNotificationPtr->mastercontrolNodeId || IdentityPtr->nodeId == nodeNotificationPtr->reservemastercontrolNodeId ||
        IdentityPtr->nodeId == nodeNotificationPtr->intragroupcontrolNodeId ||
        IdentityPtr->nodeId == nodeNotificationPtr->reserveintracontrolNodeId) {
        ...
        {
            cout << "备用控制损坏 ,暂且备份节点置0，等待下一轮节点选取" << endl;
    ...
    if (nodeNotificationPtr->nodeResponsibility == MASTERCONTROL || nodeNotificationPtr->nodeResponsibility == INTRAGROUPCONTROL) {
        ...
        vector<uint8_t> *data_control = (vector<uint8_t> *)controlmsg.data;
        unsigned int eventType1 = ((*data_control)[3] << 8) | (*data_control)[4];
        cout << "毁伤消息处理身份更新  eventType is  " << eventType1 << endl;
