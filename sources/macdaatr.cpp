...
void MacDaatr::LoadSlottable_setup() {
    string stlotable_state_filename = "./config/SlottableInitialization/Slottable_RXTX_State_" + to_string(static_cast<long long>(nodeId)) + ".txt";
    string stlotable_node_filename = "./config/SlottableInitialization/Slottable_RXTX_Node_" + to_string(static_cast<long long>(nodeId)) + ".txt";
    ifstream fin1(stlotable_state_filename);
    ifstream fin2(stlotable_node_filename);

    if (!fin1.is_open()) cout << "Cannot Open Setup File1" << endl;

    if (!fin2.is_open()) cout << "Cannot Open Setup File2" << endl;
...
#ifdef DEBUG_SETUP
    for (int i = 0; i < TRAFFIC_SLOT_NUMBER; i++) {
        if (slottable_setup[i].state == DAATR_STATUS_TX && slottable_setup[i].send_or_recv_node != 0) {
            cout << "|TX:" << slottable_setup[i].send_or_recv_node;
        } else if (slottable_setup[i].state == DAATR_STATUS_RX && slottable_setup[i].send_or_recv_node != 0) {
...
void MacDaatr::LoadSlottable_Execution() {
    cout << "-----------------------------------" << endl;
    string stlotable_state_filename = "./config/SlottableExecution/Slottable_RXTX_State_" + to_string(static_cast<long long>(nodeId)) + ".txt";
    string stlotable_node_filename = "./config/SlottableExecution/Slottable_RXTX_Node_" + to_string(static_cast<long long>(nodeId)) + ".txt";
    ifstream fin1(stlotable_state_filename);
    ifstream fin2(stlotable_node_filename);
    ...
    for (int i = 0; i < TRAFFIC_SLOT_NUMBER; i++) {
        if (slottable_execution[i].state == DAATR_STATUS_TX && slottable_execution[i].send_or_recv_node != 0) {
            cout << "|TX:" << slottable_execution[i].send_or_recv_node;
        } else if (slottable_execution[i].state == DAATR_STATUS_RX && slottable_execution[i].send_or_recv_node != 0) {
...
void MacDaatr::MAC_NetworkLayerHasPacketToSend(msgFromControl *busin) {
    ...
    */
    ...
    if ((busin->packetLength == 73 && busin->msgType == 15 && busin->fragmentNum == 15)) {
        ...
        if (busin->msgType == 2) next_hop_node = 0;
    ...
    if (MFC_temp->msgType == 1)
        ...
        if (MFC_temp->backup == 0) {
            if (MFC_temp->msgType == 3 && Forwarding_Table[MFC_temp->destAddr - 1][1] == 0)
                cout << nodeId << "->" << MFC_temp->destAddr << " 无高速链路, 丢弃数据包 (msgType = 3)" << endl;
    ...
    if (state_now == Mac_Execution) {
        ...
        if (mana_node != temp->intragroupcontrolNodeId) {
            ...
            if (nodeId == mana_node_pre) {
                ...
                // MAC_DAATR_Mana_Judge_Enter_Freq_Adjustment_Stage); MESSAGE_Send(node, send_msg, SPEC_SENSING_PERIOD * MILLI_SECOND);
...
void MacDaatr::macParameterInitialization(uint32_t idx) {
    ...
    if (!file.is_open()) cerr << "!!!!无法打开文件: " << filePath << endl;
...
// 读取文件内容
    ...
    while (getline(file, line)) {
        ...
        if (type == "SUBNET") {     // 子网号
            ...
            if (items[0] == "ORDINARY") {          // 普通节点
                node_type = Node_Ordinary;
            } else if (items[0] == "MANAGEMENT") { // 网管节点
                node_type = Node_Management;
            } else if (items[0] == "GATEWAY") { // 网关节点
                node_type = Node_Gateway;
            } else if (items[0] == "STANDBY_GATEWAY") { // 备份网关节点
                node_type = Node_Standby_Gateway;
            }
        } else if (type == "MANA_CHANEL_BUILD_STATE_SLOT_NODE_ID") { // 低频信道建链阶段初始化
            for (int i = 0; i < items.size(); i++) {
                low_freq_link_build_slot_table[i].nodeId = stoi(items[i]);
            }
        } else if (type == "MANA_CHANEL_BUILD_STATE_SLOT_STATE") { // 低频信道非建链阶段初始化
            for (int i = 0; i < items.size(); i++) {
                low_freq_link_build_slot_table[i].state = stoi(items[i]);
            }
        } else if (type == "MANA_CHANEL_OTHER_STATE_SLOT_NODE_ID") { // 低频信道非建链阶段初始化
            for (int i = 0; i < items.size(); i++) {
                low_freq_other_stage_slot_table[i].nodeId = stoi(items[i]);
            }
        } else if (type == "MANA_CHANEL_OTHER_STATE_SLOT_STATE") { // 低频信道非建链阶段初始化
            for (int i = 0; i < items.size(); i++) {
