...
bool judgeReTrans(const msgFromControl &MFC_temp, const vector<msgFromControl> &MFC_list_temp) {
    if (MFC_list_temp.size() == 0 || MFC_temp.repeat == 0) return false;

    for (auto mfc_in_list : MFC_list_temp) {
        if (mfc_in_list.packetLength == MFC_temp.packetLength && mfc_in_list.seq == MFC_temp.seq && mfc_in_list.msgType == MFC_temp.msgType &&
            mfc_in_list.priority == MFC_temp.priority && mfc_in_list.fragmentNum == MFC_temp.fragmentNum &&
...
void Business_Lead_Forward(MacDaatr *macdata_daatr, int dest_node, int prio, int location) {
    int i = location;
    while (macdata_daatr->businessQueue[dest_node - 1].business[prio][i + 1].priority != -1 && i < TRAFFIC_MAX_BUSINESS_NUMBER - 1) {
        macdata_daatr->businessQueue[dest_node - 1].business[prio][i] = macdata_daatr->businessQueue[dest_node - 1].business[prio][i + 1];
...
int getAvailableLocationOfQueue(MacDaatr *macdata_daatr, int dest_node, int pri) {
    ...
    if (dest_node != -1) {
        while (loc <= TRAFFIC_MAX_BUSINESS_NUMBER - 1) {
            ...
            if ((macdata_daatr->businessQueue[dest_node - 1].business[pri][loc].priority == -1)) break;
...
void Insert_MFC_to_Queue(MacDaatr *macdata_daatr, msgFromControl *busin, int pri, int loc, int temp, int my_pkt_flag) {
    ...
    if (my_pkt_flag == 0) {
        if (temp) {
            ...
            // 将原本优先级队列的业务后移一位
            for (i = loc; i > 0; i--)
                macdata_daatr->businessQueue[busin->destAddr - 1].business[pri][i] =
                    macdata_daatr->businessQueue[busin->destAddr - 1].business[pri][i - 1];
    ...
    } else // 这说明这是需要查看路由表后再插入队列的数据包
    {
        ...
        if (temp) {
            ...
            // 将原本优先级队列的业务后移一位
            for (i = loc; i > 0; i--)
                macdata_daatr->businessQueue[next_hop_addr - 1].business[pri][i] =
                    macdata_daatr->businessQueue[next_hop_addr - 1].business[pri][i - 1];
...
// 包含返回true
static bool Judge_If_Include_Gateway(MacDaatr *macdata_daatr, vector<LinkAssignment_single>::iterator LAtemp) {
    return (LAtemp->begin_node == macdata_daatr->gateway_node || LAtemp->end_node == macdata_daatr->gateway_node ||
            LAtemp->begin_node == macdata_daatr->standby_gateway_node || LAtemp->end_node == macdata_daatr->standby_gateway_node);
}

// 判断这个时隙序列是否可以提供给这个节点(主要用于频分复用, 在频分复用中,
// 节点号都只能在收发节点中出现一次) 传入的参数为: 待编排的业务; 时隙表;
// 业务的待分配时隙的长度;
static bool Judge_If_Choose_Slot(vector<LinkAssignment_single>::iterator LinkAssignment_single_temp, Alloc_slot_traffic Alloc_slottable_traffic[],
                                 int slot_length) {
    for (int i = 0; i < slot_length; i++) {
        if (Alloc_slottable_traffic[i].multiplexing_num == 0) return false;
...
void ReAllocate_Traffic_slottable(MacDaatr *macdata_daatr, vector<Alloc_slot_traffic> &Alloc_slottable_traffic,
    ...
    if (macdata_daatr->gateway_node != 0) rest_slot_num_per_node[macdata_daatr->gateway_node - 1] = TRAFFIC_SLOT_NUMBER * 0.6; // 暂定网关节点2
    if (macdata_daatr->standby_gateway_node != 0) rest_slot_num_per_node[macdata_daatr->standby_gateway_node - 1] = TRAFFIC_SLOT_NUMBER * 0.6;
    ...
    if (!fin1.is_open()) cout << "Cannot Open FullConnection File1" << endl;
    if (!fin2.is_open()) cout << "Cannot Open FullConnection File2" << endl;
                ...
                for (j = 0; j < temp; j++) {
                    for (k = 1; k <= macdata_daatr->subnet_node_num; k++) {
                        if (k != Alloc_slottable_traffic[i].send_node[j] && k != Alloc_slottable_traffic[i].recv_node[j] &&
                            k != macdata_daatr->gateway_node && k != macdata_daatr->standby_gateway_node) {
...
void sendLocalLinkStatus(MacDaatr *macdata_daatr) {
    ...
    for (int dest_node = 1; dest_node <= macdata_daatr->subnet_node_num; dest_node++) {
        if (macdata_daatr->businessQueue[dest_node - 1].distance != 0 && macdata_daatr->if_receive_mana_flight_frame[dest_node - 1] == true) {
...
void sendOtherNodePosition(MacDaatr *macdata_daatr) {
    ...
    for (nodeNum; nodeNum <= SUBNET_NODE_NUMBER_MAX; nodeNum++) {
        if (macdata_daatr->subnet_other_node_position[nodeNum].nodeId == 0) { // 此处截止
            // printf("nodenum is %d\n", nodeNum);
            break;
        }
    }
    ...
    int i;
    for (i = 0; i < nodeNum - 1; i++) { // 将已收到的结构体信息赋值给要发送的结构体中
    ...
    if (macdata_daatr->nodeId == macdata_daatr->mana_node) {
        ...
        for (dest_node = 1; dest_node <= macdata_daatr->subnet_node_num; dest_node++) {
            ...
            string stlotable_state_filename =
                "./config/SlottableExecution/Slottable_RXTX_State_" + to_string(static_cast<long long>(dest_node)) + ".txt";
            string stlotable_node_filename =
                "./config/SlottableExecution/Slottable_RXTX_Node_" + to_string(static_cast<long long>(dest_node)) + ".txt";
...
static int calculateFreqNum(MacDaatr *macdata_daatr) {
    ...
    for (int i = 0; i < TOTAL_FREQ_POINT; i++) {
        if (macdata_daatr->subnet_frequency_sequence[i] == 1) num++;
    }
    return num;
}
        ...
        for (int node = 0; node < 5; node++) {
            ...
            while (prev_node > 0) {
                if (freqSeq[node][time] - freqSeq[prev_node][time] <= MIN_FREQUENCY_DIFFERENCE ||
                    ...
                    if (freqSeq[node][time] >= MAX_FREQUENCY) freqSeq[node][time] -= MAX_FREQUENCY;
                } else
                    prev_node--;
            }
        }
    ...
    for (int time = 0; time < FREQUENCY_COUNT; time++) {
        for (int node = 0; node < SUBNET_NODE_NUMBER_MAX; node++) {
            ...
            while (available_frequency[current_frequency] == 0) {
                ...
                if (current_frequency >= MAX_FREQUENCY) current_frequency -= MAX_FREQUENCY;
            ...
            for (int i = node + 1; i < SUBNET_NODE_NUMBER_MAX; i++) {
                ...
                while (available_frequency[next_frequency] == 0) {
                    ...
                    // 检查频点是否超过最大频率, 若超过则减去最大频率
                    if (next_frequency >= MAX_FREQUENCY) next_frequency -= MAX_FREQUENCY;
                    if (frequency_found_count == TOTAL_FREQ_POINT) {
                        // printf("找不到可用频点\n");
                        flag = 1;
                        break;
...
static void generateFreqSequence(MacDaatr *macdata_daatr) {
    ...
    while ((narrow_band_found || unavailable_freq_found) && total_attempts < max_attempts) {
        ...
        for (int time = 0; time < FREQUENCY_COUNT; time++) {
            ...
            if (narrow_band_found || unavailable_freq_found) break;
        }
        total_attempts++;
        break;
    }
    for (int i = 0; i < SUBNET_NODE_NUMBER_MAX; i++) { // 将生成的调频数组赋值给网管节点保存的生成跳频组
...
static void generateSubnetUsedFrequency(MacDaatr *macdata_daatr) {
    int i, j;
    int temp = 0;
    for (i = 0; i < TOTAL_FREQ_POINT; i++) {             // 首先全部置0
    ...
    for (i = 0; i < TOTAL_FREQ_POINT; i++) {                        // 打印当前子网所使用频点
...
void judgeIfEnterFreqAdjustment(MacDaatr *macdata_daatr) {
    ...
    for (i = 0; i < TOTAL_FREQ_POINT; i++) {
        for (j = 0; j < SUBNET_NODE_NUMBER_MAX; j++) {
            if (macdata_daatr->spectrum_sensing_sum[j][i] == 0 &&
                macdata_daatr->spectrum_sensing_sum[j][TOTAL_FREQ_POINT] == 1) { // 若当前节点对应的频谱感知结果已经接收到了,
                // 且此频段受到干扰了
    ...
    if (using_freq_interfer_ratio >= INTERFEREENCE_FREQUENCY_THRESHOLD) {
        ...
        if (macdata_daatr->need_change_state == 0 && macdata_daatr->state_now != Mac_Adjustment_Slot &&
        ...
        } else if (macdata_daatr->need_change_state == 1) { // 在这之前已经发生了要进入时隙调整阶段, 但还未广播此信息,
            // 也即未进入
