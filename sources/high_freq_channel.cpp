...
void MacDaatr::highFreqSendThread() {
    ...
    while (true) {
            ...
            for (int i = 0; i < SUBNET_NODE_NUMBER_MAX; i++) {
                for (int j = 0; j < TRAFFIC_CHANNEL_PRIORITY; j++) {
                    for (int k = 0; k < TRAFFIC_MAX_BUSINESS_NUMBER; k++) {
                        if (businessQueue[i].business[j][k].priority != -1) businessQueue[i].business[j][k].priority = -1;
                    }
                }
            }
            continue;
        ...
        if (state_now == Mac_Initialization) {
            ...
            if (currentStatus == DAATR_STATUS_TX && dest_node != 0 && dest_node <= subnet_node_num) {
                // TX
                ...
                if (access_state == DAATR_WAITING_TO_SEND_HOPPING_PARTTERN && waiting_to_access_node == dest_node && FullConnection_slot == true) {
                ...
                for (int i = 0; i < TRAFFIC_CHANNEL_PRIORITY; i++) {
                    ...
                    for (int j = 0; j < TRAFFIC_MAX_BUSINESS_NUMBER; j++) {
                        if (businessQueue[dest_node - 1].business[i][skip_pkt].priority == -1) break;
                        ...
                        if (businessQueue[dest_node - 1].business[i][0].busin_data.backup == 0) {
                            ...
                            if (busin_data.packetLength == 73 && busin_data.msgType == 15 && busin_data.fragmentNum == 15) {
                                // 说明待发送的是频谱感知结果
                                // 设置包头
                                MacHeader *mac_header = new MacHeader;
                                mac_header->PDUtype = 0;
                                ...
                                mac_converter_Spec.daatr_MFCvector_to_0_1(MFC_Spec_temp,
                                                                          busin_data.packetLength); // 将MFC的vector加入converter的seq中
                ...
                if (nodeId == mana_node) {
                    ...
                    if (dest_node <= subnet_node_num) {
                        ...
                        for (int j = 0; j < TRAFFIC_SLOT_NUMBER; j++) {
                            New_Slottable_ptr->slot_traffic_execution_new[j] = slot_traffic_execution_new[dest_node - 1][j];
                        }
...
#ifdef DEBUG_ADJUST
                        if (dest_node <= subnet_node_num) {
                            cout << "[调整阶段 " << currentSlotId << "T] 网管节点向 Node " << dest_node << " 发送时隙表  ";
                            printTime_ms();
                        }
#endif
                        delete mac_header;
...
#ifdef DEBUG_ADJUST
                        if (dest_node <= subnet_node_num) {
                            cout << "[调整阶段 " << currentSlotId << "T] 网管节点向 Node " << dest_node << " 发送跳频图案  ";
                            printTime_ms();
                        }
#endif
                        delete mac_header;
...
void MacDaatr::highFreqChannelHandle(uint8_t *bit_seq, uint64_t len) {
    ...
    if (mac_header.is_mac_layer == 0) {            // 上层数据包 + 频谱感知
        ...
        {
            // 检查此时的接收节点是不是网管节点
            if (nodeId != mana_node) cout << "Node " << nodeId << " Wrong Recv Node of BuildLink Request!" << endl;
        ...
        {
            // 检查此时的接收节点是不是普通节点
            if (nodeId == mana_node) cout << "Node " << nodeId << " Wrong Recv Node of BuildLink Response!" << endl;
            ...
            if (blRequest.if_build == 1) {
                cout << "[" << currentSlotId << "R] Node " << nodeId << " 收到建链许可--" << mac_header.seq << "  ";
                printTime_ms();
            } else {
        ...
        {
            // 检查此时的接收节点是不是普通节点
            if (nodeId == mana_node) cout << "Node " << nodeId << " Wrong Recv Node of Slottable!" << endl;

            MacDaatr_struct_converter mac_converter_Slottable(7);
            mac_converter_Slottable - mac_converter;
            mac_converter_Slottable.daatr_0_1_to_struct();

            highFreqSlottable *slottable_ptr = (highFreqSlottable *)mac_converter_Slottable.get_struct();
            ...
            cout << endl;
            cout << "[调整阶段] Node " << nodeId << " 收到新时隙表 -- " << receivedSlottableTimes << "             ";
            printTime_ms();
        ...
        case 5: // 普通节点收到调频图案
        {
            ...
            if (nodeId == mana_node) cout << "Node " << nodeId << " Wrong Recv Node of Sequence!" << endl;
            ...
            if (mac_header.mac_pSynch == 1101) {
                ...
                cout << endl;
                cout << "[调整阶段-HIGH] Node " << nodeId << " 收到新调频图案-- " << receivedSequenceTimes << "  ";
                printTime_ms();
            }
            break;
        }
        ...
        {
            // 检查此时的接收节点是不是网管节点
            if (nodeId != mana_node) cout << "Node " << nodeId << " Wrong Recv Node of ACK!" << endl;

#ifdef DEBUG_ADJUST
            cout << "[调整阶段 " << currentSlotId << "R] 网管节点收到 Node " << mac_header.srcAddr << " 的ACK  ";
            printTime_ms();
#endif
            break;
        }
