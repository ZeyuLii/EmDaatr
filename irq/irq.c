...
#define IRQ_DRIVER_ID	1235

// 设备名称
#define MYDEV_NAME		"mydev_clk"
...
static int mydev_open(struct inode *inode, struct file *file)
{
	...
	printk("mydev_open_clk is called. \n");
	return 0;	
}
...
{
	void __user *pArg;
	//printk("mydev_ioctl_clk is called. cmd = %d \n", cmd);
	if (101 == cmd)
	{
		// 说明应用程序设置进程的 PID 
		pArg = (void *)arg;
	...
	else if (102 == cmd)
	{
		...
		// 把用户空间的数据复制到内核空间
		if (copy_from_user(&SIMULATION_TIME, pArg, sizeof(int)))
		{
		    printk("copy_from_user failed! \n");
		    return -EFAULT;
		}
...
static int __init mydev_driver_init(void)
{
	...
	printk("mydev_driver_init_clk  is called. \n");
    	IRQ_NUM = gpio_to_irq(960);
...
{	
	printk("mydev_driver_exit_clk  is called. \n");
